...
## 2026-07-18 修正頂部導覽4個下拉選單的emoji與文字對齊問題
...
## 2026-07-21 地圖探索頁新增德國地圖（Mosel／Rheingau／Pfalz／Baden）

212. **查證確認德國4個法定產區沒有專屬的產區邊界開放資料集**：GovData唯一名為「Weinanbaugebiet」的資料集實際只涵蓋薩克森一個邦（與這4個產區完全無關）；萊茵蘭-普法爾茲邦官方葡萄園登記簿API（demo.ldproxy.net/vineyards）只到單一葡萄園（Einzellage）地塊層級，沒有法定產區整體邊界，且Rheingau（黑森邦）／Baden（巴登-符騰堡邦）跨邦後資料來源不一致——確認使用者事前判斷正確，這4個產區屬風土概念而非行政區直接對應，比照法國模式（不比照義大利/西班牙的真實行政大區邊界模式）。
     原因：任務明確要求動工前先查證資料源、不可逕行假設；查證結果與使用者的事前判斷一致，但仍完整記錄查證過程與依據，避免下次誤以為「德國也有像義大利一樣的官方產區邊界」而重工。
213. **改用縣級行政區（Kreis）分組湊凸包，資料源為 `isellsoap/deutschlandGeoJSON`（GitHub，已封存但為靜態穩定版本）的 `4_kreise/3_mittel.geo.json`（中等簡化，1.9MB，與法國departements資料同一量級）**，快取為 `scripts/.geo-cache/de_kreise_mittel.geo.json`。縣分組（依Wikipedia核實的Bereich範圍推導，已與使用者確認）：Mosel=Trier-Saarburg＋Bernkastel-Wittlich＋Cochem-Zell（核心摩澤爾/薩爾河谷3縣）；Rheingau=Rheingau-Taunus-Kreis（單一縣，Wikipedia明確證實）；Pfalz=Bad Dürkheim＋Südliche Weinstraße＋Neustadt Städte＋Landau Städte（Deutsche Weinstraße核心帶）；Baden=Main-Tauber／Rhein-Neckar-Kreis／Heidelberg Städte／Karlsruhe／Karlsruhe Städte／Rastatt／Baden-Baden Städte／Ortenaukreis／Breisgau-Hochschwarzwald／Freiburg／Emmendingen／Lörrach／Waldshut／Konstanz／Bodensee共15縣（使用者明確選擇完整9個Bereich涵蓋，形狀狹長貫穿巴登-符騰堡邦西側是官方真實範圍、非疏漏，另一選項「僅取Kaiserstuhl/Ortenau/Breisgau精華3縣」使用者未採用）。
     原因：使用者提供兩個方案讓其選擇（完整涵蓋vs精華段），選擇忠於官方真實範圍優先於視覺緊湊度。
214. **`data/wine-data.js` 的4筆德國產區（mosel/rheingau/pfalz/baden）先前皆缺少 `coords` 欄位（其他三國都有才能產生地圖標記），本次新增代表座標**：Mosel→Bernkastel-Kues(49.9161,7.0694)、Rheingau→Rüdesheim am Rhein(49.9833,7.9306)、Pfalz→Deidesheim(49.4078,8.1845)、Baden→Ihringen/Kaiserstuhl(48.0440,7.6484，因Baden範圍選擇完整涵蓋、南北狹長，取最具代表性的核心酒村而非地理中心點，比照使用者確認的方案）。座標皆透過網路搜尋核實。
     原因：這是產生地圖標記的必要前置欄位，非任務外意外擴大範圍；代表座標選點原則與其他三國「選最知名產區核心酒村」的既定慣例一致。
215. **新增 `scripts/build-germany-map.pl`**：架構比照 `build-france-map.pl`（縣分組湊凸包＋全國輪廓用Natural Earth admin-0 countries資料集，與法/義/伊比利共用同一份快取），標籤避讓演算法比照 `build-iberia-map.pl`（大/小形狀依「重心到邊界平均距離÷標籤半寬」比值自適應分流：Baden形狀夠大歸類為「大形狀」，用point-in-polygon硬性容器約束；Mosel/Pfalz/Rheingau形狀狹長歸類為「小形狀」，改用位移上限+指示線)，因4個產區形狀狹長（尤其Mosel沿河谷延伸），比法國「無避讓」更適合。河流採用Natural Earth資料集裡獨立命名為"Rhine"的LineString（涵蓋緯度48.97°~51.82°，即巴登北段經萊茵高、美因茲一路到荷蘭；涵蓋不到巴登最南端近巴塞爾/Bodensee河段，此為資料源限制，比照法國「資料源沒涵蓋就不強求」的既定做法，未外插補畫）。過程中發現 `data/wine-data.js` 是CRLF換行，France/Italy/Iberia三支既有腳本的正規表示式只認`\n`，理論上重新執行會失敗——這與本次任務無關，僅附註提醒、不在本次任務內處理。
     原因：延續三支既有腳本已驗證過的方法論，不重新發明；標籤演算法選用Iberia版本是使用者明確指定的要求。
...
## 2026-07-22 修復響應式檢測報告列出的6項問題（375/390/768px）

219. **依響應式檢測報告修復6項問題，全部改動加在既有4個`@media`查詢之後的新增區塊（`css/style.css`第181行起），不改動桌面版既有樣式**：
     - **產區資料庫標題+搜尋框擠壓**：`#region-search`所在的flex容器新增`id="regions-header-row"`，`@media(max-width:600px){#regions-header-row{flex-wrap:wrap}#region-search{width:100%}}`，讓搜尋框在窄螢幕下換到獨立一行、改滿版寬度。
     - **L2大區篩選列裁切**：`@media(max-width:768px)`內把`#l2-bar.open`從`max-height:160px;overflow:hidden`改為`max-height:none;overflow-x:auto;overflow-y:hidden`，`#l2-filters`改`flex-wrap:nowrap`＋`.fp2{flex-shrink:0}`，把「裁切隱藏」改為「單行橫向捲動」，與年份矩陣既有的`.vm-wrap{overflow-x:auto}`同一種模式。
     - **地圖標記觸控熱區**：`js/map.js`的4個`renderXMarkers()`（France/Italy/Iberia/Germany）在每個`<g class="pulse-marker">`內新增一個`<circle class="tap-target" r="22" fill="transparent">`作為第一個子元素，視覺完全不變（transparent無可見填色），因`onclick`綁在外層`<g>`上、點擊會自然冒泡，不需另外綁事件。已用`document.elementFromPoint()`實測確認距標記中心14個SVG單位處（原本8px可見圓點之外、新22px熱區之內）能正確命中`tap-target`。
     - **年份矩陣捲動提示＋欄名稱裁切**：新增`js/vintage.js`的`updateVMScrollHint()`，在`.vm-wrap.scrollWidth>clientWidth`時加上`has-overflow`class顯示右側漸層陰影提示，捲到底時加`scrolled-end`隱藏；`buildVintageMatrix()`結尾呼叫。
     - **欄名稱裁切根因**（**已修正報告初稿的錯誤假設**）：實測診斷（比對捲動前後`.rl`與`.vm-region-group-hdr`的`getBoundingClientRect()`）確認`.rl`（每列「Bordeaux Left Bank／波爾多左岸」區域標籤）的`position:sticky`完全正常、捲動前後座標不變；真正裁切的是大區分隔列「波爾多 BORDEAUX」，該儲存格用`colspan="${totalCols}"`跨滿全部欄位，且是`position:static`。進一步實測發現**Chrome對「跨滿全部欄位的colspan儲存格」套用`position:sticky`完全不生效**（不論是td本身還是td內的inline-block子元素，捲動時都跟著內容整個位移，與正常寬度的`.rl`行為完全不同——同一個`.vm-wrap`容器內插入一個普通非表格`<div>`測試sticky則正常運作，證實問題出在colspan跨欄儲存格本身，而非容器）。修法：把`js/vintage.js`裡該列HTML從單一`colspan="${totalCols}"`儲存格拆成兩個td——第1欄`.vm-region-group-hdr`不跨欄（與`.rl`同寬同邏輯），`@media(max-width:768px)`內設`position:sticky;left:0;z-index:1`；其餘欄位改用新增的`.vm-region-group-hdr-fill`（colspan填滿剩餘欄位，僅延續背景色與底線，不含文字）。視覺上兩個td背景色相同、無縫銜接，桌面版完全看不出差異。
     - **比較模式品種下拉選單截斷**：發現使用者提供的選擇器`#cm-mode-appellation > div[style*="..."]`因產地模式的grid外層多包一層`#cm-appellation`（品種模式沒有這層）而無法命中，改為在4個目標div（產地A/B卡片列、品種A/B卡片列、`#compare-identity`、`#compare-grape-identity`）加上共用class`cm-two-col`，`@media(max-width:520px){.cm-two-col{grid-template-columns:1fr!important}}`（inline style需要`!important`才能覆蓋）。
     - **品種圖鑑雷達圖標籤裁切**：`js/grapes.js`的`buildGrapeRadar()`新增`const containerWidth = ctx.parentElement.clientWidth`，容器寬度<300px時`pointLabels.font.size`用10、否則維持原本的13，取代原本寫死的13。
     原因：動工前逐一view現況確認6項描述與實況相符（除4B、5兩項回報有出入並取得使用者確認調整方向後才動工），所有CSS修法都比照既有的md媒體查詢寫法風格、JS修法則是既有函式的最小增補，不引入新的抽象或渲染模式。
     驗證：headless Chrome於375/390/768px三種寬度＋1280px桌面寬度分別截圖+DOM量測確認：(1)375px下`body.scrollWidth`從399降回等於視窗寬度375，標題從4行斷行降為2行；(2)L2大區篩選列改為單行橫向捲動，原本375px下完全不可見的Alsace/Loire Valley捲動後可見且可點擊（`curL2`確認）；(3)`document.elementFromPoint()`確認擴大後的觸控熱區確實能命中原本點不到的位置；(4)年份矩陣「波爾多 BORDEAUX」等大區分隔列捲動前後`getBoundingClientRect().left`完全不變（29→29），捲到底時陰影提示正確消失；(5)比較模式品種A/B卡片改為單欄堆疊後，下拉選單完整顯示「Cabernet Sauvignon(卡本內蘇維濃)」不再截斷；(6)品種圖鑑雷達圖容器寬度280px（375px視窗下）時7個維度標籤（含右側Acidity/Body）全部完整顯示、無裁切。1280px桌面寬度下逐項確認：L2篩選列恢復多行wrap（`overflow-x:hidden`）、比較模式恢復雙欄（`614.5px 614.5px`）、雷達圖容器恢復340px用13px字級、標題與搜尋框恢復同列，皆與修復前桌面版外觀完全一致、無回歸。

## 2026-07-22 產區資料庫擴充：奧地利Kamptal/Burgenland、葡萄牙Dão/Alentejo共4筆

220. **`data/wine-data.js` 的 `WINE_DB.appellations` 新增4筆產區，皆插入至對應國家既有區塊最後一筆之後**：Austria新增 `kamptal`（坎普塔爾，緊鄰Wachau、綠維特利納與麗絲玲並重）與 `burgenland`（布根蘭，藍佛朗克紅酒與新希德爾湖貴腐甜酒雙重門面），插入於 `wachau` 之後；Portugal新增 `dao`（當歐，花崗岩山區優雅版國產多瑙加紅酒）與 `alentejo`（阿連特茹，溫暖平原現代餐酒＋羅馬陶罐talha古法並存），插入於 `vinho-verde` 之後。動工前已view核對 `wachau`／`douro` 現有欄位順序，確認與使用者提供的4筆新資料格式一致（`coords`欄位wachau本身沒有但douro有，屬既有「coords預留給未來地圖擴充」慣例的延續，非結構出入，已於動工前向使用者回報並取得確認）。
     原因：使用者提供奧地利/葡萄牙擴充清單，欄位格式逐字比照已驗證通過的既有產區結構，內容由使用者提供、未自行增刪。
     **意外但正面的發現**：Portugal的 `dao`／`alentejo` 兩筆雖如使用者所述「目前無對應GeoJSON地圖」，但因西班牙/葡萄牙共用同一張Iberia地圖（`js/map.js` 的 `getIberiaAppellations()` 篩選條件為 `country`為Spain或Portugal 且有 `coords`），這兩筆新增了真實座標後**自動被納入現有Iberia地圖**，以編號12、13正確顯示在地圖上對應的葡萄牙地理位置、側邊清單同步新增對應分組，非本次任務刻意實作，而是既有地圖渲染邏輯對新資料的正確反應；Austria因完全沒有對應地圖（France/Italy/Iberia/Germany四張皆不含奧地利），`kamptal`／`burgenland`維持與`wachau`相同的既有「缺少地圖座標」清單狀態，不受影響。
     驗證：Perl統計全檔案大括號/中括號開合數一致（1178/1178、626/626），確認4筆逐一插入未破壞JSON結構；`auditWineDB()` 主控台輸出確認106/106完全通過相關檢查（`missingCompare`為空，`franceUnboundToVintage`與改動前相同）、「缺少地圖座標」清單新增`kamptal`／`burgenland`兩筆（性質與既有`wachau`相同，非新問題），`dao`／`alentejo`則因自動掛上Iberia地圖而未出現在缺少地圖座標清單中；headless Chrome分別展開Austria（3個L2大區群組，含2筆新增：Kamptal/Burgenland）與Portugal（4個L2大區群組，含2筆新增：Dão/Alentejo）分頁，並實際開啟Kamptal與Alentejo兩筆的完整詳情抽屜，確認品種標籤、風味輪、物理結構量化7項（含profile數值比對）、辨識特徵、餐酒配對、風土、代表酒莊、歷史背景、陳年潛力解析等全部欄位皆正確渲染、無undefined或缺漏；額外截圖確認Dão/Alentejo正確以編號12/13顯示在Iberia地圖上、地理位置與側邊清單分組皆正確。

## 2026-07-29 釀造工藝頁6款式production欄位結構化重構（table/steps雙版型＋品種連結）

221. **`data/wine-data.js` 的 `WINE_DB.wineStyles` 6個物件各新增 `productionType`／`productionSteps`或`productionTable`／`tags`／`linkedGrapes` 4個欄位，原本的 `production` 純文字欄位完整保留不刪除**（作為完整敘述備份，前端改顯示結構化版型取代原本直接輸出這段文字的區塊）；`history`／`grapes`／`terroir` 三欄位文字完全不動。sparkling／rosé／fortified 三款用 `productionType:'table'`（對照表），red／white／orange 三款用 `productionType:'steps'`（編號流程）。內容與各欄位分組（如sparkling的傳統法/水槽法/轉注法三欄對照、red的6步驟流程）皆由使用者逐字提供，未自行增刪。
     - **`js/winestyles.js` 新增 `buildProductionTableHTML()`／`buildProductionStepsHTML()` 兩個渲染函式**：table版型用深酒紅色（`var(--burg)`）標題列＋既有白卡配色；steps版型用深酒紅色圓底白字編號（與既有`classifications.js`的tiers金色文字編號刻意做視覺區隔，避免使用者混淆「分級層級」與「釀造流程」兩種不同性質的編號列表）。`buildWineStyleCardHTML()` 依 `s.productionType` 分流呼叫。
     - **tags 沿用 `js/regions.js` 既有的 `tg tg-trait`**（keyIdentifiers pill樣式），未新增CSS class。
     - **linkedGrapes 沿用 `js/regions.js` 既有的 `tg tg-grape` + `jumpToGrapeById()` 可點擊連結寫法**（比照該檔案primaryGrapes欄位「先查`WINE_DB.grapes`是否存在對應id、存在才做超連結」的既定模式，差別是這裡的id由使用者直接提供並在動工前逐一核對過，不需再動態查找）；`linkedGrapes`為空陣列（orange／fortified兩款）時，整個「相關品種」區塊完全省略，不輸出任何空狀態文字。
...
## 2026-07-29 新增「儲存與侍酒」頁面；核對分級制度頁術語覆蓋率與釀造工藝頁字數/技術內容（依WSET L2官方規格Issue 2, 2023）

227. **新增「儲存與侍酒 Storage & Service」面板，比照07-11食物搭配面板的建置方式（`#panel-storage`＋導覽下拉第三項，`switchToPanel`/`showPanel`通用比對邏輯不需修改）**：內容為5張`.acc-wrap`手風琴卡片（複用`toggleSATSection`，不新增JS渲染邏輯，資料直接寫死在`index.html`）——理想儲存條件（溫度10–15°C求穩定勝於精確值、光害Lightstrike、瓶身平躺防軟木塞乾裂、濕度50–80%）、開瓶後保存方式（真空幫浦vs惰性氣體系統如Coravin）、建議侍酒溫度（6大類共6個溫度區間，官方LO6僅給定性描述，此處溫度數字為WSET教材/業界慣用具體值）、開瓶與醒酒程序（靜態酒/氣泡酒開瓶差異、醒酒兩種目的的時機差異、杯型與SAT外觀氣味檢查）、常見酒缺陷關聯（軟木塞污染/封瓶失效/熱害）。
     原因：使用者依WSET L2官方規格核對後認定平台缺少LO6（儲存與侍酒服務）對應內容，要求新增；侍酒溫度區間官方規格不含精確攝氏度數，已在動工前明確告知使用者這部分需自行依業界慣用值補上，非官方硬性規定的數字。
228. **核對過程發現：「品飲系統」頁既有「常見酒缺陷」清單僅5項（軟木塞污染/氧化/揮發性酸過高/還原/過量SO2），並無「熱害」項目，與使用者原始需求「熱害可連結呼應既有卡片」的前提不符**，取得使用者確認後同步在品飲系統頁的常見酒缺陷清單（`index.html`內嵌HTML，非獨立`js/tasting.js`資料）新增第6項「熱害 Heat Damage (Maderisation)」缺陷卡片（沿用既有左邊框色塊+線索/成因格式，橘色`#D4801A`與既有5色區隔），並新增`js/core.js`的`jumpToFaultById(id)`函式（比照`jumpToGrapeById`寫法：`switchToPanel('tasting')`＋開啟`#sat-nose-hdr`手風琴＋`scrollIntoView`）建立儲存頁↔品飲系統頁的雙向連結，兩個既有缺陷卡片（軟木塞污染/氧化）與新增的熱害卡片皆補上`data-fault-id`供函式查找。
     原因：先核對再動工，不可假設需求描述的前提（既有卡片可連結）等於現況；使用者選擇「同步在品飲系統頁新增熱害卡片」而非讓儲存頁獨立說明不連結。
     驗證：headless Chrome確認`#panel-storage`啟用後含5張`.acc-wrap`；手風琴單開行為正確（開啟第1卡自動收合第3卡）；點擊儲存頁3個「前往品飲系統頁」連結皆正確切換至`panel-tasting`、展開嗅覺手風琴、並將對應`data-fault-id`元素捲入可視範圍（`getBoundingClientRect().top`介於0與視窗高度之間）；反向由品飲系統頁熱害卡點擊連結正確切回`panel-storage`；字級層級逐層核對：h1(26px)＞手風琴標題(20px)＞`.ins-lbl`小標(17px)／副標(17px)＞內文(16px)，符合CLAUDE.md字級層級鐵則；`typeof jumpToFaultById`為function、`document.querySelectorAll('[data-fault-id]').length`為3，確認無遺漏。
229. **核對「分級制度」頁（`js/classifications.js`＋`WINE_DB.classifications`12筆資料）對WSET官方25項標籤術語的覆蓋率，純核對不動程式碼**：已涵蓋7項（義大利DOCG/DOC/IGT、西班牙DOCa/DO/VdlT、勃根地Cru、德國Prädikatswein全6子級+Eiswein、西班牙Joven-Reserva體系、義大利Riserva）／部分涵蓋7項（EU PDO/PGI、法國AOC/AOP/IGP、德國Qualitätswein/Landwein、Botrytis/Noble Rot、波爾多Cru Bourgeois等、Trocken/Halbtrocken）／完全缺漏11項（Old Vine、Late Harvest、阿爾薩斯Grand Cru、Valpolicella/Soave的Classico、氣泡酒Brut/Demi-Sec/Cap Classique全3項、雪莉Fino-PX全項、波特Ruby-Tawny全項）。缺漏集中在氣泡酒/加烈酒的風格標示與各國陳年/風格副標示，多半原本就不在現有12張卡片「By酒莊/葡萄園/產區」三分類架構的涵蓋範圍內。
     原因：使用者要求逐條核對回報、不擅自修復；每一項狀態皆用`grep`實際核對`WINE_DB.classifications`資料內文（非憑印象判斷），部分涵蓋/完全缺漏的判定標準統一為「是否已是清單裡的獨立tier或僅在敘述文字帶過」。
230. **核對「釀造工藝」頁6款式`history`/`grapes`/`terroir`欄位字數與sparkling/fortified技術內容深度，純核對不動程式碼**：確認DECISIONS.md並未像產區資料庫「150-200字」那樣留下可查閱的明確數字標準——#74僅記載「其餘四欄依使用者給定的字數區間撰寫」但未寫下實際數字。實測現況（HTML標籤已排除）：`history` 156–203字、`grapes` 131–155字、`terroir` 97–149字（rose的terroir 97字明顯短於其餘5款）。技術內容比對LO5規格：fortified完整涵蓋雪莉flor生物陳年/氧化陳年差異與波特中止發酵機制；sparkling涵蓋Chardonnay/Pinot Noir/Meunier/Glera與傳統法/水槽法/轉注法三法比較，但**完全未提及Moscato／Asti風格**（LO5明確列名的氣泡酒品種之一），是本次核對抓到的唯一實質內容缺口。
     原因：使用者要求先核對現況與比對官方考綱重點再決定是否修改，不直接假設既有內容完整；字數與技術覆蓋率兩者皆先呈現實測結果供使用者判斷後續是否需要統一基準或補寫Moscato/Asti內容。
231. **使用者裁定不統一terroir欄位字數基準（維持現況），但確認補寫#230抓到的Moscato/Asti內容缺口**：在`data/wine-data.js`的`WINE_DB.wineStyles[0]`（sparkling）`grapes`欄位末尾新增一段說明——Asti／Moscato d'Asti以`<span class="grape-inline-link">`連結既有`muscat`品種卡片，內容涵蓋Moscato Bianco品種本身、僅單次發酵並提前冷卻/過濾中止（與傳統法/水槽法/轉注法的「二次發酵」邏輯本質不同）、保留大量天然糖分與濃郁葡萄香/荔枝/橙花氣息、酒精度遠低於其他氣泡酒（約5–6% abv）等WSET教材標準事實；`grapes`欄位字數因此從147字增為316字（使用者已明確表示不需要統一字數基準，此差異不視為需修正的問題）。
     原因：延續本專案既有的23品種行內連結名單（`muscat`已在#222確認存在且用於white款），新增連結未擴大名單範圍；技術內容（單次發酵/低酒精度/中止時機）皆為WSET Level 2標準教材事實，無不確定的具體數字。
     驗證：headless Chrome確認全域大括號/中括號配對仍平衡（1207/1207、653/653）；`winestyles`面板sparkling卡片展開後`grapes`段落正確顯示新增文字；點擊Moscato連結正確切換至`panel-grapes`並展開`muscat`品種卡片（`.acc-hdr[data-grape-id="muscat"]`確認`open`）。

## 2026-07-29 品種資料庫擴充：WSET L2 LO4「區域重要品種」10個缺席品種（依WSET Level 2官方規格Issue 2, 2023核對）

232. **動工前逐一核對10個候選品種對應產區的現況，發現3項與使用者原始清單描述不符**：使用者原本預期「應完全不存在」的Montepulciano d'Abruzzo（id: `montepulciano-dabruzzo`）與未明確歸類的Verdicchio dei Castelli di Jesi（id: `verdicchio-dei-castelli-di-jesi`）**其實已存在**，屬於「品種缺席但產區已存在」而非「兩者皆缺席」；Carmenère的既有資料庫拼法是雙重音符「**Carménère**」而非使用者訊息的單一重音符「Carmenère」，首輪grep因拼法不同而漏抓，改用「Central Valley」關鍵字才找到`maipo-valley`/`colchagua-valley`兩筆既有產區；Pinotage對應的「Western Cape」本身不是獨立產區id（只是`region`分類欄位），實際持有Pinotage的產區是`stellenbosch`。最終分類：6個品種（Corvina／Garganega／Montepulciano／Verdicchio／Pinotage／Carménère）僅需新增品種資料並連結既有產區；4個品種（Barbera／Cortese／Fiano／Furmint）需同時新增品種與產區資料。
     原因：延續「先逐項核對現況是否仍成立，再動工，不可假設清單描述等於現況」的既有方法論；本次核對抓到的3處落差若未事先核對，會導致重複新增本已存在的產區資料，或用錯誤拼法漏掉既有連結對象。
233. **新增`wsetLevel:'2-regional'`欄位區分WSET LO3（現有8個核心品種）與LO4（本次新增10個區域重要品種）**：`css/style.css`新增`.tg-lo4`（沿用`.tg`基礎樣式，金色系與既有LO3的`.tg-match`紅色系區隔），`js/grapes.js`新增對應徽章渲染條件，10個新品種全部標記此欄位並顯示「WSET L2·LO4」徽章。使用者在此議題的兩個選項（新增語意欄位／完全不區分）中選擇前者。
     原因：使用者認為LO3（38%）與LO4（24%）雖同屬必考範圍但權重與性質不同，資料層面精確區分對準備考試更有價值；**現有23筆品種裡先前擴增的15個非LO3品種，尚未逐一核對哪些屬於官方LO4其餘12個品種而應一併回溯標記，此為本次任務範圍外的已知缺口，留待日後單獨處理，不在本次擴大稽核**。
234. **新增4筆產區資料，義大利3筆／匈牙利1筆（網站首筆匈牙利資料）**：Barbera d'Asti(`barbera-dasti`)與Gavi(`gavi`)插入於既有`asti`（Moscato d'Asti所在產區）之後，同屬皮埃蒙特但為不同DOCG；Fiano di Avellino(`fiano-di-avellino`)插入於Campania的Taurasi之後；Tokaj(`tokaj`)新增獨立的Hungary區塊，插入於Navarra之後（全陣列最後一筆）。因應匈牙利為首次出現的國家，同步新增`assets/flags/hu.svg`（紅白綠三橫條，色碼比照官方旗幟色標）與`js/core.js`的`COUNTRY_FLAG_CODE`新增`'Hungary(匈牙利)':'hu'`一筆。
     原因：試點筆（Barbera d'Asti）取得使用者確認格式後才批次處理其餘3筆，比照07-08既定流程；匈牙利國旗與對照表是否一併處理已於動工前明確詢問使用者並取得確認，不逕行假設。
...
245. **使用者回報第一排酒色篩選（全部/紅葡萄/白葡萄）與第二排分級篩選視覺不一致：字級疑似不同、且第一排缺少數量顯示**：實際檢查computed CSS後確認`font-size`（16px）／`font-weight`／`padding`／`letter-spacing`兩排完全相同，並無真正的字級差異，回報使用者後確認觀感落差來自第一排含🔴🟡表情符號（視覺上比純文字顯眼）加上第二排文字較長（含數量）兩者疊加的錯覺，非CSS臭蟲。
     - 唯一真正可修正的落差——第一排缺少數量顯示——改為將`grape-color-filters`從`index.html`裡的靜態3個按鈕，比照`renderGrapeTierFilters()`的既有寫法，改成`renderGrapeColorFilters()`動態渲染（數字依`WINE_DB.grapes`即時計算`skinColor`分布，不寫死），架構上與第二排完全對稱一致。
...
## 2026-08-28 Phase 2：新增模擬考UI與計分邏輯（`js/quiz.js`）

262. **等級徽章不沿用既有`.sbadge`/`.sbl`/`.sbo`/`.sbe`/`.sbg`，另建`.quiz-badge`＋`.qb-dist`/`.qb-merit`/`.qb-pass`/`.qb-fail`/`.qb-unclassified`5個修飾class**：圓形版型（50px、2.5px border、`'Cinzel'`字體）與`.sbadge`完全比照，但字級改用`var(--fs-card-title)`而非既有`.sbadge`寫死的`15px`；配色沿用`.sbl`/`.sbo`/`.sbe`/`.sbg`「rgba背景＋border-color＋color三件式」的既有寫法慣例，Distinction用`var(--gold-dk)`、Merit用`var(--burg)`、Pass用`var(--em)`（三者皆為既有CSS變數），Fail因站內色板無現成警示色新增`#C87020`（與`js/map.js`隆河谷地圖區塊既有的橘色同色系）、Unclassified用既有`var(--txt3)`灰階表示「未達標」最弱等級。
     原因：`.sbadge`系列語意上綁定「單一分數評分」（如年份評級1-5分），與模擬考5級精熟度分級（Distinction~Unclassified）是不同的語意分類系統，共用同一組class會讓未來修改其中一種評分邏輯時互相牽連；徽章內容比照`js/vintage.js`既有「圓形短碼＋旁邊完整文字標籤」的呈現模式（短碼放`D`/`M`/`P`/`F`/`U`，完整雙語分級名稱放在旁邊的文字區塊），而非把長分級文字硬塞進圓形內。
263. **選項洗牌採「開始測驗當下才對抽出的50題各自建立新物件」，不直接修改或引用`QUIZ_BANK`原始物件**：`buildQuizQuestionSet()`對每題的`options`陣列位置索引做Fisher-Yates洗牌後，用洗牌後的索引序列重新組出新的`options`陣列與新的`correctIndex`，回傳全新物件、`QUIZ_BANK`本身不被讀寫以外的操作觸碰。
     原因：此為runtime呈現邏輯，與題庫產出時「正解位置須用程式化隨機分佈、不可人工排列」的[[quiz-data生產規則]]（DECISIONS.md #261）是兩個不同階段的規則，不可混淆；用複製新物件而非原地修改，確保重新測驗、或使用者中途重整頁面時，`QUIZ_BANK`全域陣列本身永遠維持題庫原始狀態，不會被前一次測驗的洗牌結果污染。
264. **及格判定用「答對題數÷50」的精確浮點數與5個級距門檻比較，不先四捨五入成整數百分比再比較**：`calculateQuizResults()`回傳的`scoreRatio`直接參與`>=0.85`/`>=0.70`/`>=0.55`/`>=0.45`門檻判斷，畫面顯示的百分比（`toFixed(1)`）僅是判斷完成後另外算的呈現用字串，不回頭影響分級結果。
     原因：50題滿分下每題等於2%，四捨五入到整數或一位小數在邊界分數（如27/50=54.0%應為Fail、非四捨五入誤判為Pass的55%）可能造成分級誤判；比較邏輯與顯示邏輯分離，確保「使用者看到的百分比」和「實際決定等級的數字」永遠是同一組運算結果，不會出現顯示55%卻因四捨五入前是54.6%而被判Fail的不一致情況。
265. **5級分級門檻（≥0.85 Distinction／≥0.70 Merit／≥0.55 Pass／≥0.45 Fail／<0.45 Unclassified）直接寫死為常數，未做成可設定資料**：對應WSET_L2Wines_Specification_EN_May2023_Issue2第19頁官方配分表訂定的門檻，屬於官方規格的固定事實而非站內可調參數。
...
## 2026-08-28 修正模擬考頁面文案與實際儲存行為不一致

274. **修正index.html兩處「本次成績不會儲存」文案**（起始畫面規則說明、結果畫面分級摘要下方提示），改為準確反映#271-273已導入的localStorage歷史紀錄行為：起始畫面（僅描述exam模式規則）改為「成績會保留在本機瀏覽器最近10筆歷史紀錄，可於「查看歷史成績」查閱」；結果畫面（exam/practice共用同一段靜態文字）改為「模擬考成績會保留在本機瀏覽器最近10筆歷史紀錄；LO練習模式不會儲存」。
     原因：這兩處文案是在歷史成績功能（#271-273）加入之前就寫好的，功能上線後未同步更新，導致使用者截圖詢問「頁面說不會儲存，但『查看歷史成績』又是幹嘛用的」──這是文案與程式行為脫節的真實矛盾而非誤解。結果畫面沿用單一靜態文字（不依quizMode動態改字）而非在renderQuizResults()另外分支處理，是因為一句話已能同時把兩種模式的實際行為講清楚，不需要為此新增JS邏輯，維持本次修正只動文案、不動程式邏輯的最小範圍。

## 2026-08-28 修正quiz-state-lo-select畫面缺少返回按鈕（使用者反映live網站卡住無法離開）

275. **在`quiz-state-lo-select`（選LO練習畫面）的LO按鈕群組下方，補上一顆「返回」按鈕（`onclick="showQuizState('start')"`），樣式比照`quiz-state-history`既有的返回按鈕**：這是Phase 2新增LO篩選練習模式那次（DECISIONS.md #266-270）的遺漏——當時只顧著加6個LO選擇按鈕，沒有同步比照後來`quiz-state-history`補上的返回按鈕模式。
     原因：使用者實際使用live網站時回報「點進選LO練習後卡住，切到其他分頁再切回模擬考、或點導覽列的模擬考分頁都回不去正式模擬考起始畫面」——因為`showPanel()`只切換panel層級的顯示，不會重置panel-quiz內部三態畫面（`.quiz-state`）目前停在哪一個，`quiz-state-lo-select`本身又沒有任何離開路徑，兩者疊加造成使用者被困在此畫面，是會直接卡住使用者的真實功能缺陷，不是文案問題。

## 2026-08-28 模擬考起始畫面排版整理＋新增LO官方配分明細

276. **起始畫面規則說明從單一`<p>`＋`<br>`手動換行改為真正的`<ul><li>`清單，並新增「各Learning Outcome配分 Official LO Allocation」明細區塊（6個LO各自的題數與佔比＋合計列，用簡單的兩欄flex列表呈現，不用`<table>`）**：文字內容於動工前先與使用者確認過（含各LO題數/佔比是否與`QUIZ_LO_ALLOCATION`常數一致：5/4/19/12/6/4，佔比10%/8%/38%/24%/12%/8%），確認無誤才落地實作。
     原因：使用者回報原本用單一段落塞`<br>`手動分行的寫法排版混亂（換行後的接續行沒有對齊清單縮排），改用`<ul><li>`讓瀏覽器原生處理清單縮排與換行對齊；LO配分明細改用flex兩欄列表而非`<table>`，是因為表格在窄螢幕行動裝置容易溢出需要額外處理橫向捲動，而flex兩欄列表天生响應式、且與結果畫面既有的LO統計呈現手法（`quiz-lo-breakdown`）視覺語彙一致，不需要引入新的排版模式。
277. **起始畫面的「開始測驗／選LO練習／查看歷史成績」三顆按鈕，從各自寫死`margin-left:10px`改為外層包一個`display:flex;flex-wrap:wrap;gap:14px`容器**：卡片寬度同步從`max-width:520px`加大到`560px`，讓新增的LO配分明細有足夠寬度不至於太擁擠。
     原因：使用者反映三顆按鈕距離太近；`margin-left`只解決相鄰按鈕的間距、且在窄螢幕換行時第二排按鈕會貼齊左邊界而非維持一致間距，改用flex容器的`gap`可以確保換行前後間距一致，且未來如需再增減按鈕數量時不必逐一調整每顆按鈕自己的margin。

## 2026-08-28 修正測驗規則清單無項目符號＋卡片加寬

278. **`quiz-state-start`規則清單`<ul>`補上`list-style-type:'・ ';`，卡片`max-width`從720px前的560px加大到720px**：#276把規則說明改成語意化的`<ul><li>`後，項目符號完全沒有顯示；追查發現本站有載入完整版Tailwind CDN（`<script src="https://cdn.tailwindcss.com">`），其preflight會全域清空`<ul>`/`<ol>`的`list-style`，此前站內從未用過真正的`<ul><li>`（既有內容一律用手動「・」字元或div排版），因此這個副作用先前未被踩到。
     原因：既然要用語意化清單就要讓符號實際顯示，`list-style-type:'・ '`（CSS字串型marker）比改回`list-style:disc`更符合站內既有的「・」符號視覺慣例，維持風格一致；卡片加寬是因為使用者反映4點規則中有3點在560px寬度下會換行、閱讀吃力，加大到720px讓多數句子單行顯示，仍保持置中卡片的版面設計，不改成滿版寬度。

## 2026-08-28 修正「選LO練習」畫面6個LO按鈕排列不整齊

279. **`quiz-state-lo-select`的6個LO按鈕，容器從`display:flex;flex-wrap:wrap;justify-content:center`改為`display:grid;grid-template-columns:repeat(2,1fr)`兩欄等寬網格，每顆按鈕補上`width:100%;text-align:left;`**：原本用flex+置中排列時，每個按鈕寬度依文字長度各自撐開（`.fp`的`white-space:nowrap`），6個LO標籤長短不一（如「LO3 主要品種」4字 vs「LO1 葡萄種植環境因素」8字），造成左右不對齊、視覺凌亂；使用者兩次追加反映同一問題（先是「排列很亂」要求統一寬度，接著即使等寬了、文字仍置中顯示時又反映「因為每個LO文字長度不一樣看起來還是亂」）。
     原因：改用grid兩欄讓所有按鈕寬度強制一致（不受文字長短影響），文字再改成靠左對齊（而非沿用置中），是因為等寬容器內如果文字仍置中，短標籤（如「LO3 主要品種」）與長標籤（如「LO1 葡萄種植環境因素」）的文字起始位置仍會參差不齊，只有靠左對齊才能讓6個標籤的文字起點對齊成一條線，徹底解決「看起來亂」的根本原因，而不只是解決按鈕外框寬度不一致的表面問題。

## 2026-08-28 修正作答畫面(quiz-state-active)缺少離開路徑，正式考與LO練習皆會卡住
...
283. **白葡萄品種色條顏色從`var(--gold-dk)`（#A88A60）改為`var(--gold)`（#C5A880）**：使用者反映深金色（--gold-dk）與酒紅色（--burg）在4px窄色條上不夠好區隔，要求調淺。
     原因：`var(--gold)`（#C5A880）正是CLAUDE.md鎖定慣例明文列出的那個金色本尊（「背景 #FAF8F5、酒紅 #5C061C、金色 #C5A880」），比原本選用的`--gold-dk`變體更淺、也更直接對應規範裡指名的顏色，一次同時解決「不夠淺」與「更貼近鎖定色票」兩件事；已用Playwright實測確認改色後渲染為`rgb(197,168,128)`。

## 2026-09-06 buildQuizQuestionSet()加入runtime no-run約束，並修正參考演算法的尾端盲區

284. **`js/quiz.js`的`buildQuizQuestionSet()`在回傳前新增`breakQuizCorrectIndexRuns()`後處理，搭配`quizHasRunAt()`輔助函式，偵測並打散洗牌後仍連續3題以上`correctIndex`相同的排列**：題庫檔案本身雖已在建置時做過no-run約束（見#284前的一系列quiz-data.js收尾驗收記錄），但這個約束只涵蓋靜態檔案排列，`buildQuizQuestionSet()`每次洗牌題目順序與選項順序時完全沒有這層防護，使用者依此規格提供參考演算法（單次交換、只找`j>i`的候選）並要求依此實作。
     原因：只調整題目在陣列中的排列順序（透過交換兩個已隨機化好的題目物件），不改變任何一題個別的選項洗牌邏輯或`options`內容，維持「選項洗牌」與「題序排列」兩個獨立步驟互不干擾；選在`buildQuizQuestionSet()`回傳前套用，讓50題模擬考與8題單LO練習兩種呼叫情境（共用同一支函式）都自動受益，不需要在`startQuiz()`/`startPractice()`個別呼叫端重複處理。
285. **實測發現使用者提供的參考演算法只往後（`j>i`）搜尋交換候選，導致連續3題落在陣列尾端時結構性無法修正，並非使用者原先預期的「極端邊界情況」**：驗證腳本對50題allocation跑1000次，92次仍殘留違規，其中73/113個違規位置精確落在陣列最後一格，其餘也集中在倒數1-8格內，與「隨機4選項序列最後3題剛好同答案」的理論機率（1/4²=6.25%）高度吻合，證實這是演算法方向性設計造成的系統性盲區而非機率極低的意外；因此在往後搜尋找不到候選時，新增往前（`j<i-2`）搜尋作為備案，兩段搜尋各自沿用原有「交換→驗證→不符合就換回」的邏輯不變。
     原因：使用者提供的參考邏輯本身有這個結構缺口，若未經驗證直接依樣實作並回報「已完成」會是不實的驗收結果；把發現的問題誠實回報並提出具體數據佐證後，由使用者決定要擴充搜尋方向還是接受原樣，而非自行默默修改或默默照單全收，是遵守「有明顯矛盾或錯誤時回報而非自行修改」慣例的具體實踐。
286. **擴充往前搜尋後，50題模擬考的殘留違規率從約7-9%降至2000次驗證0次（0%），8題單LO練習仍殘留約1.15%-2.05%（6個LO分別測試皆同一量級），使用者確認接受此殘留、不再進一步處理**：debug發現8題模式的殘留成因與尾端盲區不同，是抽到的8題`correctIndex`分布極端集中時（如8題中6題同答案位置），需要同時移動多個題目才能完全打散，已超出「單次交換」演算法能處理的範圍。
     原因：50題模擬考是有官方5級分級、會被記入歷史成績的正式情境，優先確保它完全收斂到0%殘留；8題練習模式賭注低（不計分、不記錄），且其殘留成因（多題必須同時重排）如果要徹底解決，需要引入更複雑的重排邏輯（例如允許連續多次交換或整體打散重排），這正是使用者一開始就明確要求不要為了處理的「找不到可交換對象的邊界情況」，只是原先誤判其發生機率，釐清後仍落在使用者原本設定的「不需要特殊處理」範圍內，維持單次交換的演算法簡潔性優先於徹底根除機率極低的殘留。

## 2026-09-06 及格分級5級制查證為WSET官方標準（非本專案自創設計）

287. **確認`js/quiz.js`裡的5級及格分級門檻（`calculateQuizResults()`裡`≥0.85`卓越／`≥0.70`優異／`≥0.55`合格／`≥0.45`不合格／`<0.45`未達標）與WSET官方規格書完全一致，非本專案自行延伸或過度設計**：查證來源為WSET Level 2 Award in Wines官方規格書（2019年版，wsetglobal.com/media/6942/wset_l2wines_specification_en_may2019.pdf，第17頁「2.3 Results are graded as follows」），官方原文對照表如下：

     | 分數區間 | 官方英文名稱 | 程式內對應等級 |
     |---|---|---|
     | 85%以上 | Pass with distinction | dist（卓越） |
     | 70%–84% | Pass with merit | merit（優異） |
     | 55%–69% | Pass | pass（合格） |
     | 45%–54% | Fail | fail（不合格） |
...
## 2026-09-06 LO練習模式加入歷史紀錄（獨立storage key，不與模擬考共用10筆上限）

292. **新增`QUIZ_PRACTICE_HISTORY_STORAGE_KEY`（`wineAtlasQuizPracticeHistory`）與獨立上限`QUIZ_PRACTICE_HISTORY_MAX=20`，新增`savePracticeHistoryRecord(result, lo)`/`loadPracticeHistory()`兩個函式，完全不修改既有`saveQuizHistoryRecord()`/`loadQuizHistory()`/`QUIZ_HISTORY_STORAGE_KEY`/`QUIZ_HISTORY_MAX`（模擬考那組維持10筆上限原樣不動）；`submitQuiz()`原本`if (quizMode === 'exam') saveQuizHistoryRecord(...)`改為`exam`/`practice`兩個並列分支，exam分支呼叫的函式與參數完全未變動**：練習紀錄的record結構比照既有模擬考record的欄位設計（`timestamp`/`correct`/`total`/`scoreRatio`/`loStats`），額外多一個`lo`欄位記錄本次練習的LO編號，且刻意不寫入`grade`（因為練習模式本來就不套用官方5級分級，見DECISIONS.md #268）。
     原因：練習模式預期使用頻率遠高於模擬考（8題、不計時、門檻低，適合零碎時間反覆練習），若與模擬考共用同一個10筆上限的storage key，練習紀錄會很快把模擬考的正式成績紀錄擠出裁切範圍，讓「模擬考歷史成績」這個原本就有的功能實質上失去意義；分開成獨立key與獨立上限（20筆，練習頻率高且要為未來LO趨勢分析保留較多樣本，但仍設上限避免localStorage無限成長）從根本上避免這個排擠問題，也完全不觸碰已測試穩定的既有模擬考儲存邏輯。
293. **`renderQuizHistoryList()`重構為「模擬考成績」＋「LO練習紀錄」兩個獨立區塊，各自有各自的空狀態提示文字，而非單一混合列表＋單一通用「尚無紀錄」訊息**：模擬考區塊沿用原本的卡片渲染邏輯與樣式（分級文字仍用`var(--burg)`酒紅色），練習紀錄區塊比照相同卡片外觀但右側只顯示題數與百分比（用`var(--txt2)`而非酒紅色，刻意做出「非官方分級」的視覺區隔），LO名稱透過既有`QUIZ_LO_LABELS`對照顯示中文全名而非純數字。
     原因：使用者原本提出的方案是修改單一「尚無歷史紀錄」訊息文字使其涵蓋兩種情境，但改成分區塊呈現後，讓每個區塊各自準確反映自己的資料狀態（模擬考0筆時顯示「尚無模擬考紀錄」、練習0筆時顯示「尚無練習紀錄」）比硬湊一句同時涵蓋兩種情境的通用文字更精確，也更不容易重蹈先前「文案與程式行為脫節」的問題（見#274）；右側顏色刻意用`--txt2`而非`--burg`是延續結果畫面（`renderQuizResults()`）「practice模式不假裝有官方分級」的既定設計原則，維持全站「模擬考才有分級視覺」的一致性。
294. **手動驗證：撰寫Playwright腳本模擬三種情境並實際檢查localStorage內容與畫面文字**：①完成一次LO2練習8題，確認`localStorage`裡`wineAtlasQuizPracticeHistory`寫入的紀錄`lo:2, correct, total:8, scoreRatio, loStats`皆正確，且「查看歷史成績」畫面確實顯示「LO2 釀造工藝與瓶陳」與「/ 8」字樣；②完成一次模擬考，確認`wineAtlasQuizHistory`與畫面上的分級文字（如Pass with Merit）顯示邏輯與本次改動前完全一致；③連續完成22次LO6練習，確認練習紀錄裁切為20筆、同時模擬考紀錄筆數不受任何影響（改動前後皆為1筆）。全部3項情境皆通過，過程中console/page皆無錯誤訊息。
     原因：裁切邏輯（`history.length > MAX`）與「兩個key互不干擾」都是只能透過實際寫入多筆資料才能驗證的行為，單看程式碼容易誤判裁切門檻的邊界（例如第20筆與第21筆的判斷時機），必須實際跑滿22次練習才能確認第21、22次觸發時真的正確裁掉最舊的紀錄而非模擬考的紀錄被誤裁。

## 2026-09-06 新增LO正確率彙總長條圖（模擬考＋練習歷史合併統計）

295. **新增`computeLoWeaknessStats()`彙總模擬考（`loadQuizHistory()`）與練習（`loadPracticeHistory()`）兩份歷史紀錄的`loStats`，依LO編號合併累加`correct`/`total`，固定回傳LO1~LO6六個key（即使某個LO完全沒有任何紀錄也回傳`{correct:0, total:0}`而非省略該key）；新增`renderLoWeaknessChart()`用Chart.js畫長條圖，X軸為`QUIZ_LO_LABELS`轉出的中文全名，長條用酒紅色`#5C061C`，另疊加一個line型dataset畫55%官方合格門檻的金色`#C5A880`虛線參考線（不安裝annotation外掛，用兩個dataset疊圖的最簡單作法達成同樣視覺效果）**：模擬考與練習兩種情境的答錯資料合併統計、不分開算，因為使用者在哪種情境答錯同一類題目，反映的都是同一個知識弱點，區分情境對「找出弱項」這個目的沒有意義，只會徒增圖表複雜度。
     原因：固定回傳LO1~LO6六個key（而非只回傳有資料的LO）是為了讓X軸永遠是完整的6欄，不會因為使用者剛好還沒練習過某個LO就讓那根長條「消失」而不是「顯示為0%」，兩者對使用者的閱讀體感差異很大——消失容易被誤讀成「圖表壞了」，顯示0%才能正確傳達「這個LO還沒有資料，不代表你這個LO很強」的訊息。
296. **`loWeaknessChartInst`比照既有`radarInst`（`js/compare.js`）/`grapeRadarInsts`（`js/grapes.js`）的Chart.js實例管理慣例，存成模組層級變數，`renderLoWeaknessChart()`一開始就先呼叫既有實例的`.destroy()`再建立新的；渲染呼叫點放在`renderQuizHistoryList()`函式結尾（而非另外在`showQuizState()`裡加一行呼叫），確保只要進入歷史成績畫面就一定連動更新圖表，不需要記得在兩個地方各自維護觸發時機**：若沒有偵測到任何模擬考或練習紀錄（兩份history皆為空陣列），改為隱藏圖表容器、顯示提示文字，不渲染一張全部是0%的空白長條圖。
     原因：全部0%的空圖表雖然技術上「有資料」，但對使用者而言在視覺上會被誤解為「你所有LO都考0分」，比起完全不顯示圖表的空狀態訊息更容易造成誤解，尤其這是使用者第一次點進歷史成績畫面時最可能遇到的情境；把渲染呼叫點放在`renderQuizHistoryList()`內部而非額外在`showQuizState()`加一行，是因為`renderQuizHistoryList()`本身就是這個畫面資料變動的唯一權威來源，讓圖表渲染跟著它的既有生命週期走，避免未來有人修改`showQuizState()`時漏改圖表這一行造成資料不同步。
297. **本次刻意排除「近N次模擬考LO3正確率隨時間變化」這類時間趨勢折線圖，只做「目前為止」的單一彙總長條圖**：時間趨勢圖需要額外的資料切片邏輯（依時間範圍篩選、依單一LO篩選）與互動設計（讓使用者選LO、選時間區間），這些需求本質上是另一組獨立的UI元件與資料處理邏輯，複雜度與工作量都與本次單純彙總長條圖不對等。
     原因：這是使用者在需求描述裡就明確排除的範圍，記錄下來是為了避免未來有人看到「LO正確率分析」這個功能名稱，誤以為理所當然應該要有時間趨勢，而在沒有重新評估範圍與工作量的情況下就直接動手擴充，忽略了這是經過討論後刻意暫緩、留給未來獨立評估的項目，而非遺漏或半成品。
298. **手動驗證：撰寫Playwright腳本涵蓋4種情境並直接讀取Chart.js實例內部資料**：①完全清空`localStorage`後進入歷史成績畫面，確認提示文字（「完成至少一次模擬考或練習後...」）顯示、圖表容器隱藏；②完成2次LO1練習＋1次LO3練習＋1次模擬考後，直接在瀏覽器內呼叫`computeLoWeaknessStats()`與讀取`loWeaknessChartInst.data.datasets[0].data`比對兩者數值是否一致，實測結果六個LO的正確率分別為`[23.8, 0, 22.2, 25, 16.7, 25]`，與手動彙總換算結果逐一比對後完全相符；③確認`dataset[1]`（55%參考線）六個值皆為`55`；④連續進出歷史成績畫面5次後，檢查Chart.js全域`Chart.instances`裡綁定在該canvas上的實例數量固定為1，未隨進出次數增加，確認舊實例確實被正確`destroy()`、無疊圖或記憶體洩漏。全程console/page皆無錯誤訊息，並截圖確認長條圖／參考線／圖例／既有歷史列表版面皆正常顯示無跑版。
     原因：Chart.js實例是否正確銷毀、圖表資料是否與底層資料源同步，都不是單看程式碼就能百分之百確認的行為（尤其`.destroy()`遺漏在某些情境下不會立即報錯、只會隨使用次數緩慢累積記憶體），必須實際反覆操作瀏覽器並讀取`Chart.instances`這個底層API才能確認没有疊圖，這也是延續本專案一貫「瀏覽器互動行為一律用Playwright實測、不能只憑程式碼推論」的既定做法（見DECISIONS.md #291）。

## 2026-09-08 修正quiz-data.js中27題錯誤引用網站自訂profile評分系統的出題方式（commit b68180c、9d6fe72）

299. **`data/quiz-data.js`裡LO3的22題（commit `b68180c`）與LO4的5題（commit `9d6fe72`），全數移除「profile」「評分」「幾分」及具體數字分數語彙，改用定性描述或原理性問法重新出題；correctIndex以node腳本重新程式化生成（0-3均勻分佈、shuffle），非人工排列**：`data/wine-data.js`的`WINE_DB.grapes[品種id].profile`是網站自製的0-10分內部評分物件，設計初衷僅供品種雷達圖視覺呈現使用（見DECISIONS.md #1，commit `aebceb8`：`tannin`/`acidity`/`floral`屬品種相對固定的基因特性，`body`/`alcohol`/`finish`/`aging`則隨產區風土劇烈浮動），並非WSET官方標準、也不是可考的客觀事實，但quiz-data.js過去有27題直接把這組內部數字當成正解依據出題（如「依profile資料，Cabernet Sauvignon的單寧評分為何？」選項給「8分」）。
     原因：使用者明確指出這27題的出題邏輯本身有誤——考生答對答錯只是背下了這個網站自己給的參考數字，不是真正理解葡萄酒知識；逐題查證後，內容判斷責任由使用者下放給本次對話，凡查證後底層知識點真實可靠者保留並改寫（如白酒不帶皮發酵故單寧極低、厚皮晚熟品種單寧較重等原理性事實），凡屬「純粹是資料庫內部數字排列結果、且沒有可對應的真實知識點」者則替換為其他確定知識點或錨定回LO1氣候與成熟度原理，過程中5題（見#300逐題對照表中的mer-012/syr-011/pgg-012/mer-014/syr-014）因與更廣泛的真實案例（如波爾多右岸頂級Merlot陳年實力、北隆河冷涼Syrah風格、阿爾薩斯晚摘Pinot Gris陳年力）有潛在矛盾，先行列出回報給使用者確認處理方向（分別採用「錨定LO1氣候概念」「限定產區對比」「替換為其他確定知識點」三種對應方案）後才動工，未自行判斷硬改。
300. **改寫完成後，另比照#284-286的no-run鐵律精神，額外延伸適用於「原地置換既有題目」的情境**：新增node腳本（`shuffle_correct.js`），先取出這27題（扣除僅改explanation、選項與correctIndex皆未變動的lo3-cha-003後為26題）在原300題全陣列中緊鄰的前後題目correctIndex，組成多個segment序列，其中待重新分配的題目標記為`X`，以均勻分布的候選池（26個值分4組各6-7個）隨機洗牌後檢查每個segment是否有連續3個以上相同值，不合格則整批重新洗牌重試，直到找到合法解為止（本次第6次嘗試即找到）。
     原因：既有no-run鐵律（#284-286）原本只處理「新增題目批次」與「單次runtime洗牌」兩種情境，從未處理過「在既有陣列中原地置換一批分散各處的舊題目」這種情境——若只確保這27題彼此之間不連續3題同答案，置換後仍可能與陣列中緊鄰的其他273題舊題目意外湊成新的連續3題，因此把檢查範圍延伸至每題的實際陣列鄰居；改寫完成後以node腳本讀入完整300題陣列做最終驗證（VM沙盒解析語法、結構檢查全數通過、27題查無profile/評分/數字分殘留、300題id全數唯一、全陣列correctIndex連續3題以上同值殘留數為0、整體0-3分佈為75/76/72/77），全數通過。

301. **逐題修改前後對照表（供使用者不必重讀程式碼即可複核判斷）**：

     | id | 修改前問法要點 | 修改後問法要點 | 保留/捨棄理由 |
     |---|---|---|---|
     | lo3-cha-003 | 題幹不變，僅explanation提到「tannin profile為0」 | explanation改為「白葡萄酒普遍不進行帶皮發酵，單寧含量極低，為所有白葡萄品種共通特性」 | 僅語彙置換，底層事實（白酒不帶皮發酵故單寧低）真實可靠，保留 |
     | lo3-cha-010 | 問「Chardonnay單寧評分為何」，選項給「0分」等 | 改問「為什麼白葡萄酒幾乎不帶單寧口感」，定性作答 | 底層原理（無皮渣接觸故單寧低）真實可靠，保留並定性化 |
     | lo3-cha-011 | 問「酸度7分、陳年潛力6分」，explanation暗示「橡木桶陳年版本才更會陳年」 | 改問「未橡木桶高酸型（如Chablis特級園）vs 商業化早飲橡木型」的陳年力對比 | 原本「橡木桶＝更會陳年」的因果有誤（頂級Chablis不經橡木桶陳年力反而極強），比照pgg-012同一標準改寫，此判斷已於本次對話向使用者複查並取得同意後執行 |
     | lo3-svb-011 | 問「酸度評分為何」，選項給「9分」 | 改問「天然酸度表現屬於哪種等級」，定性作答 | 白蘇維濃高酸為業界公認事實，保留並定性化 |
     | lo3-svb-012 | 問「陳年潛力評分與Chardonnay相比」，給「3分vs6分」 | 改問「兩者陳年潛力的一般傾向」，並加註「多數」「尤其新世界芳香奔放型」等hedge語 | 底層傾向（多數SB為保鮮不做長陳年）為常見WSET教學觀點，加hedge後保留 |
     | lo3-rie-010 | 問「酸度與白蘇維濃相比」，強調「兩者評分皆為9分」數字相同 | 改問「兩者在酸度表現上的共同點」，答「皆是公認酸度最突出的代表」 | Riesling與SB同屬高酸品種代表為業界共識，拿掉「數字剛好相同」的暗示後保留 |
     | lo3-rie-011 | 問「陳年潛力評分為何」，給「7分」 | 改問「為什麼清淡風格仍以陳年力聞名」，答案指向高酸支撐陳年結構 | Riesling高酸支撐陳年、發展汽油礦石調性為WSET經典案例，保留 |
     | lo3-rie-014 | 問「單寧與酒體評分分別為何」，給「0分、3分」 | 改問「單寧與酒體的典型表現」，定性描述「單寧極低、酒體輕盈」 | 白酒單寧低、Riesling輕酒體皆為確定事實，保留並定性化 |
     | lo3-pgg-012 | 問「陳年潛力評分為何」，籠統給「3分，偏低」未區分風格 | 改問「Pinot Gris/Grigio為何陳年印象截然不同」，限定「義大利日常型 vs 阿爾薩斯晚摘/貴腐型」對比 | 原題未區分風格易誤導（晚摘/貴腐型陳年力其實很強），依使用者指示採方案(b)限定產區對比後保留 |
     | lo3-pgg-013 | 問「酸度評分與白蘇維濃相比」，給「5分vs9分」 | 改問「酸度表現與Sauvignon Blanc相比如何」，定性答「明顯低於」 | Pinot Gris/Grigio中性偏中酸、明顯低於SB高酸為業界共識，保留並定性化 |
     | lo3-cs-010 | 問「單寧評分為何」，給「8分」 | 改問「單寧為何以緊實厚重聞名」，答案指向厚皮晚熟 | Cabernet Sauvignon厚皮晚熟致高單寧為確定事實，保留並定性化 |
     | lo3-cs-011 | 問「與Merlot陳年潛力評分相比」，給「9分vs7分」 | 改問「一般而言哪一個陳年潛力較高」，加註「通常」「一般教學上」等hedge語 | CS結構通常優於Merlot為常見WSET教學傾向，加hedge、承認例外後保留 |
     | lo3-cs-014 | 問「花香評分僅2分」，數字化描述 | 改問「風味核心為何較少帶花香調性」，定性作答 | CS以黑色果香/雪松/石墨為主、花香非重點為確定事實，保留並定性化 |
     | lo3-mer-011 | 問「單寧評分與CS相比」，給「6分vs8分」 | 改問「單寧口感與CS相比通常有何差異」，定性答「較圓潤柔和」 | Merlot單寧較CS圓潤柔和為業界共識，保留並定性化 |
     | lo3-mer-012 | 問「酒體、酒精評分分別為何」，給「7分、7分」固定值 | 改問「在溫暖產區種植時，酒體與酒精濃度的變化趨勢」，錨定LO1氣候/成熟度原理 | body/alcohol隨風土劇烈浮動，無固定基因錨點，原題把單一固定值當普遍事實不成立；依使用者指示採方案(a)錨回LO1氣候概念後保留 |
     | lo3-mer-014 | 問「陳年潛力評分為7分，不如CS」 | 改問「單寧總量不算最低，為何口感仍以圓潤聞名」，答案指向早熟致單寧成熟度較高 | 原「不如CS」的跨品種陳年力比較與波爾多右岸頂級Merlot(如Pétrus)實際陳年實力衝突；依使用者指示採方案(c)換成單寧成熟度此一確定知識點 |
     | lo3-pn-012 | 問「單寧評分為何，與CS（8分）相比」，給「4分」 | 改問「單寧為何普遍輕盈，與CS形成對比」，答案指向薄皮 | Pinot Noir薄皮致單寧輕盈為確定事實，保留並定性化 |
     | lo3-pn-013 | 問「花香評分為8分，在所有品種中屬於什麼水準」 | 改問「花香表現上的品種特色」，定性答「鮮明，常與玫瑰香氣呼應」 | Pinot Noir花香（玫瑰調性）鮮明為確定事實，保留並定性化 |
     | lo3-syr-011 | 問「酒體、酒精評分分別為何」，給「8分、8分」固定值 | 改問「在溫暖氣候種植時，酒體與酒精濃度的變化趨勢」，錨定LO1氣候/成熟度原理 | 同mer-012問題性質（body/alcohol隨風土浮動），依使用者指示採方案(a)錨回LO1氣候概念後保留 |
     | lo3-syr-012 | 問「單寧評分與CS（8分）相比」，給「7分」 | 改問「單寧表現與CS相比如何」，定性答「同屬厚重，強度相近」 | Syrah/Shiraz單寧厚重、與CS相近為業界共識，保留並定性化 |
     | lo3-syr-014 | 問「陳年潛力評分為8分，接近CS」 | 改問「常帶黑白胡椒香氣與什麼因素最相關」，答案指向Rotundone化合物 | 原「接近CS」的陳年力比較對New World易飲型Shiraz不成立；依使用者指示採方案(c)換成Rotundone胡椒香此一確定知識點 |
     | lo3-sup-001 | 問「四品種中哪個單寧評分最高」，逐一列數字（CS8/Syrah7/Merlot6/PN4） | 改問「依業界公認的品種特性，何者以單寧最厚重紮實聞名」，拿掉數字 | 查證CS>Syrah>Merlot>PN的單寧強度排序為WSET等級教學普遍認可的順序，非本資料庫獨創排名，保留排名但改用業界用語 |
     | lo4-san-004 | 問「酸度評分為8分，與配餐定位關聯」 | 改問「高酸度與配餐定位的關聯」，拿掉數字 | Sangiovese高酸配番茄料理為確定WSET知識點，保留並定性化 |
     | lo4-neb-004 | 問「陳年潛力評分為10分（滿分），代表什麼」，「優於所有其他品種」 | 改問「為何以驚人長期陳年潛力聞名」，「極高單寧與酸度提供結構支撐」 | Nebbiolo(Barolo/Barbaresco)陳年力查證為業界公認經典案例，保留但拿掉「滿分/優於所有」的絕對化語氣 |
     | lo4-zin-003 | 問「酒精評分為9分，是所有品種中數一數二高」 | 改問「為何常以偏高酒精濃度聞名」，答案指向果串成熟不均、晚採收 | Zinfandel高酒精查證為公認現象（果粒成熟不均致晚採收），保留但軟化跨品種絕對排名語氣 |
     | lo4-gew-003 | 問「花香評分為9分，是所有品種中花香最不明顯/最鮮明的一個」 | 改問「為何是少數盲飲可憑香氣辨識的品種」，答案指向荔枝/玫瑰/薑調性 | Gewürztraminer高辨識度香氣查證為業界公認最鮮明代表，保留但軟化超級比較級措辭 |
     | lo4-bar-003 | 問「酸度評分為9分，與日常餐酒定位關聯」 | 改問「高酸度與日常餐酒定位的關聯」，拿掉數字 | Barbera高酸配日常餐酒為確定知識點，保留並定性化 |

     原因：使用者要求「內容判斷責任下放給你」必須搭配「事後可不重讀程式碼即可複核」的配套，此表逐題列出修改前後問法要點與保留/捨棄理由，讓使用者能快速核對每一題的判斷邏輯是否合理，而非只能翻閱程式碼diff自行推敲改動意圖；表中5題（mer-012/syr-011/pgg-012/mer-014/syr-014）與cha-011皆為本次對話中先行列出疑慮、經使用者明確回覆處理方案（a/b/c三種）後才動工的項目，特別標註方案來源以利追溯。

## 2026-09-08 修正lo3-syr-002一個可被邏輯刪去法秒殺的爛干擾選項

302. **`data/quiz-data.js`的`lo3-syr-002`（Syrah親本DNA分析題），把第4個干擾選項「至今尚無任何DNA分析研究，起源仍完全成謎待解」換成「親本為神索與慕維得爾，推翻了美國加州起源說」，題幹、其餘3個選項、correctIndex（維持0）與explanation皆未變動**：使用者實測模擬考時發現，這個干擾選項的文字直接否定了題幹已經明講的前提（題幹已說「2000年代DNA親緣分析確認」，選項卻答「尚無任何DNA分析研究」），屬於不需要任何葡萄酒知識、純靠邏輯就能刪去的爛選項，稀釋了這題真正要測驗的知識點（Syrah真正親本為Dureza與Mondeuse Blanche，而非流傳的波斯/敘利亞起源說）。
     原因：新的干擾選項比照另外兩個原有干擾選項的格式（「親本為X與Y，推翻了Z起源說」），選用神索與慕維得爾（皆為真實存在、確實常與Syrah並列於南隆河/GSM混調的品種，但並非Syrah真正親本）維持「聽起來合理但錯誤」的干擾強度，不再自我矛盾；因為只置換一個干擾選項文字、correctIndex位置未變，不影響no-run鐵律（該題前後鄰居的correctIndex排列完全未受影響），故本次未重新執行correctIndex重排腳本。

## 2026-09-08 全題庫掃描並修正42題「題幹已預設事實、選項卻直接否定」的爛干擾選項（commit 0d06100）

303. **以「題幹本身已明講或強烈預設某個事實/關係存在（如『差異為何』『互補方式為何』『為何以…聞名』『混調搭檔』等句式），干擾選項卻直接否定這個前提」為判準（與lo3-syr-002同一種問題），對全部300題跑關鍵詞掃描（`從未`/`完全無關`/`完全相同…沒有`/`毫無關聯`/`純屬巧合`/`不可能`/`完全不具備`等模式）後人工逐條複核，找出42題共約54個干擾選項符合此模式，全數只置換選項文字，題幹、其餘選項、correctIndex皆未變動**：使用者在lo4-sem-002（「Sémillon與Sauvignon Blanc混調時，兩者角色互補的方式為何」，選項卻寫「兩者歷史上從未一起混調過」）發現這不是lo3-syr-002的單一個案，要求全面掃描。掃描結果分兩層信心：32題屬「題幹明文預設事實、選項直接否定」的高信心案例（如lo3-svb-006、lo3-syr-005、lo4-fur-003等「混調角色互補」型；lo1-021、lo3-cs-003等「差異/關聯為何」型），另10題屬「型態類似但未直接否定題幹明文」的較低信心案例（如lo3-cs-014「花香表現與品種風味特徵完全無關，純屬隨機」，屬於給錯誤因果解釋而非否定題幹本身），經使用者確認「全部42題一次處理」後，兩層一併修正。
     原因：這42個新干擾選項全數改寫為「需要真實葡萄酒知識才能排除」的具體錯誤主張（例如錯誤的品種歸因如lo4-gew-003新選項把Gewürztraminer的辨識香氣誤植為Cabernet Sauvignon的黑醋栗雪松調性、錯誤的因果解釋如lo5-for-004把馬德拉酒加熱陳年工藝誤植為模仿香檳工法而非真正的赤道運輸意外起源、錯誤的具體數字如lo5-spk-015把Demi-Sec殘糖標準寫成與Extra Dry混淆的12–17g/L），而非「完全無關/完全相同/從未…過」這類純靠邏輯推理、不需任何品飲或品種知識就能刪去的空泛否定句式；只置換選項文字、不動correctIndex位置，因此不影響既有no-run鐵律，本次未重新執行correctIndex重排腳本。
304. **本次42題中有7題（lo3-rie-011、lo3-cha-011、lo3-pgg-012、lo4-san-004、lo4-zin-003、lo4-gew-003、lo4-bar-003）是本次對話稍早修正27題profile評分系統問題時（見DECISIONS.md #299-301，commit `b68180c`/`9d6fe72`）自己新犯的同一種錯誤，其中lo4-zin-003的「這是資料庫錯誤，酒精濃度不可能這麼高」與已修正的lo3-syr-002「至今尚無任何DNA分析研究」是同一種寫法（用「否定題幹已明講的事實」當作安全的萬用干擾選項模板），在同一次對話中重複犯錯**：修正profile評分系統題目時，為了避免選項洩漏數字分數，慣性套用「與品種特性完全無關」「純屬巧合」這類看似安全的萬用否定句式，卻沒有意識到這類句式本身就是另一種可被邏輯直接刪去的干擾選項瑕疵。
     原因：如實記錄這7題是自己的錯誤而非原始題庫的既有問題，避免未來查閱DECISIONS.md時誤以為#299-301的27題修正已經是「乾淨」的最終版本；也作為往後改寫干擾選項時的具體提醒——安全的干擾選項標準不是「聽起來像正經的錯誤答案」，而是「除非具備該品種的實際知識，否則無法單靠題幹文字邏輯刪去」，兩者是不同的檢查標準，需要分開驗證。

305. **逐題新舊干擾選項對照表（42題，僅列被置換的選項；未列出的選項與correctIndex皆未變動）**：

     | id | 舊干擾選項（自我矛盾） | 新干擾選項（需知識才能排除） |
     |---|---|---|
     | lo1-017 | 熟度變化與酒的風格品質完全無關 | 熟度只會影響單寧含量，酸度並不會隨之改變 |
     | lo1-021 | 產量高低與葡萄品質完全無關，毫無關聯 | 產量高低純粹取決於天氣，與修剪、種植密度等人為管理無關 |
     | lo2-012 | 與甜酒的釀造工藝完全無關的技術 | 透過額外添加糖漿調整甜度，並非透過中止發酵達成 |
     | lo3-svb-005 | 兩者風格完全相同，沒有明顯可辨識差異 | 兩者主要差異其實在於甜度高低，與香氣風格無關 |
     | lo3-svb-006 | 兩者角色完全相同，混調並無實質意義／兩者風格完全互斥，歷史上從未一起混調過 | 賽美蓉主要負責提升整體酒精濃度，白蘇維濃負責提供甜度／兩者混調主要是為了降低生產成本，與風味互補無關 |
     | lo3-svb-012＊ | 兩者陳年潛力完全相同，並無明顯差異／陳年潛力純粹取決於裝瓶年份，與品種完全無關 | 兩者陳年潛力都極高，皆適合長期窖藏／陳年潛力主要取決於封瓶方式（旋轉瓶蓋或軟木塞），與品種本身較無關 |
     | lo3-rie-011＊ | 陳年潛力與品種特性完全無關，純屬裝瓶方式差異／Riesling其實不具備陳年潛力，只是常被誤傳 | 陳年潛力主要來自其偏高的殘糖，與酸度高低較無關／Riesling的高酸主要用於平衡甜度，與陳年潛力並無直接關聯 |
     | lo3-pgg-012＊ | 陳年潛力與釀造風格完全無關，純粹取決於裝瓶年份／兩者陳年潛力其實完全相同，並無風格差異 | 純粹因為義大利與法國消費者的口味偏好不同，與實際釀造工法無關／義大利Pinot Grigio才具備長期陳年實力；阿爾薩斯版本反而多走清淡早飲路線 |
     | lo3-cs-003 | 與土壤條件完全無關，純屬種植傳統 | 偏好石灰岩台地，因排水較差、能保留更多水分供應根系 |
     | lo3-cs-014 | 花香表現與品種風味特徵完全無關，純屬隨機 | 花香調性主要來自橡木桶陳年賦予，與品種本身的香氣物質較無關 |
     | lo3-cha-011＊ | 陳年潛力與酸度高低完全無關，純粹取決於裝瓶年份 | 陳年潛力主要取決於裝瓶時添加的二氧化硫劑量，與酸度高低較無關 |
     | lo3-pn-013 | 花香表現與品種香氣特徵一般被認為完全無關／Pinot Noir完全不具備任何花香調性 | 花香調性只出現在經橡木桶陳年的版本，未經橡木桶則幾乎不存在／其香氣調性其實更接近Cabernet Sauvignon的黑色果香系，而非花香 |
     | lo3-syr-005 | 兩者角色完全相同，混調並沒有實質分工意義／希哈與格那希兩者歷史上從未一起混調過 | 兩者混調主要是為了掩蓋希哈的青澀單寧，而非追求風味互補／兩者混調比例通常固定為各半，並無主導與輔助之分 |
     | lo3-syr-012 | 兩者單寧表現被認為毫無相似之處 | Syrah/Shiraz單寧反而遠高於Cabernet Sauvignon，是所有品種中最厚重的 |
     | lo4-san-004＊ | 酸度表現與食物搭配一般被認為沒有任何關聯 | 高酸度使其特別適合搭配濃郁甜點，而非一般鹹食料理 |
     | lo4-neb-004 | 陳年潛力與單寧酸度被認為完全無關，純屬巧合現象 | 陳年潛力主要來自其偏高的殘糖，與單寧酸度結構較無關 |
     | lo4-zin-003＊ | 這是資料庫錯誤，酒精濃度不可能這麼高／酒精濃度高低與品種特性一般被認為完全無關 | 因為釀酒師習慣額外添加烈酒調整酒精濃度，與品種本身特性無關／高酒精濃度只出現在少數酒莊的頂級款，並非這個品種普遍常見的特性 |
     | lo4-gew-003＊ | Gewürztraminer完全沒有任何花香調性 | 主要因其帶有濃郁的黑醋栗與雪松調性，而非花果香氣 |
     | lo4-bar-003＊ | 酸度表現與其作為日常餐酒的定位一般被認為完全無關 | 親民日常定位主要來自其低廉的生產成本，與酸度風味較無直接關聯 |
     | lo4-mal-003 | 兩者風格完全相同，沒有可辨識的差異 | 兩者主要差異其實在於甜度，阿根廷版本明顯偏甜型 |
     | lo4-zin-001 | 純屬美國本土原生品種，與歐洲品種完全無關 | 法國的Merlot、西班牙的Garnacha兩者 |
     | lo4-vio-001 | 與現今種植面積相同，從未減少過 | 約100公頃，雖有減少但仍維持一定種植規模 |
     | lo4-vio-002 | 這種做法只存在於歷史文獻，實際上從未真正實行過 | 主要目的是為了增加紅酒的顏色深度，代表產區為Hermitage |
     | lo4-sem-001 | 與貴腐黴感染完全無關，僅因香氣濃郁被選用／只用於氣泡酒基酒，與索甸貴腐甜酒完全無關 | 更容易感染灰黴病（非貴腐黴），因此多用於快速汰除的劣質酒款／主要用於提升貴腐甜酒的清爽酸度，酒體與蜂蠟質地反而來自白蘇維濃 |
     | lo4-sem-002 | 兩者角色完全相同，混調並沒有實質分工意義／賽美蓉與白蘇維濃兩者歷史上從未一起混調過 | 賽美蓉主要負責提升整體甜度，白蘇維濃負責提升酒精濃度／混調比例通常以白蘇維濃占絕大多數，賽美蓉僅作微量調味使用 |
     | lo4-sem-003 | 賽美蓉完全不具備任何陳年潛力可言之高（原文本身有殘留錯字） | 香氣平淡是釀造失誤所致，與陳年與否沒有直接關聯 |
     | lo4-alb-002 | 名稱完全相同，兩地之間沒有任何差異 | 名稱雖相同，但兩地種植的其實是基因上完全不同的變異品系 |
     | lo4-alb-003 | 兩者風味幾乎完全相同，沒有可辨識差異 | 兩者主要差異其實在於酒精濃度高低，與香氣風味調性較無關 |
     | lo4-cor-003 | 兩者建議侍酒溫度完全相同，皆為14–15°C／侍酒溫度與工法差異完全無關，純憑個人喜好 | 兩者建議侍酒溫度確實不同，但差異主要來自瓶身大小，而非酒體結構的濃縮程度／侍酒溫度純粹取決於年份新舊，與釀造工法差異無關 |
     | lo4-fia-003 | 兩者風味完全相同，僅產區不同而已之別／兩者皆完全不具備任何陳年實力可言 | 兩者主要差異其實在於甜度，Verdicchio明顯偏甜型／兩者陳年實力主要來自偏高的殘糖，而非香氣物質本身 |
     | lo4-fur-003 | 兩者角色完全相同，可以互相替代使用／Furmint與Hárslevelű兩者從未一起混調過 | 兩者角色互補主要體現在甜度而非酸度骨架，Furmint其實負責提供圓潤甜感／兩者混調比例通常各占一半，並無明顯的主導與輔助品種之分 |
     | lo5-spk-001 | 與溫度或發酵中斷完全無關，純粹人工注入氣體 | 源自運送途中船隻長途搖晃震盪，意外產生的物理現象 |
     | lo5-spk-005 | 與其他氣泡酒工法完全相同，沒有任何差異 | 透過額外添加天然氣泡水達到起泡效果，而非透過發酵過程產生二氧化碳 |
     | lo5-spk-006 | 氣候條件與基酒的酸度需求完全沒有關聯 | 偏好日照時數最長的氣候，因為能加速糖分累積、提高基酒濃縮度 |
     | lo5-spk-015 | 兩者殘糖標準其實完全相同、沒有差異 | Demi-Sec殘糖12–17g/L，僅略高於Brut標準 |
     | lo5-for-004 | 與溫度或海運完全無關，純粹為節省釀造成本 | 源自19世紀根瘤蚜蟲害後，為了模仿香檳工法而刻意發展出的技術 |
     | lo5-for-007 | 四個品種釀造出的甜度完全相同，沒有光譜差異 | 四個品種由不甜到極甜的實際排序，與一般認知的順序恰好完全相反 |
     | lo5-for-008 | 風土與糖分濃縮完全無關 | 使葡萄單寧含量大幅降低，整體風格趨於清淡 |
     | lo5-for-009 | 與陽光反射水分保留完全無關 | 主要透過提高土壤含鹽量來抑制葡萄過度成熟，藉此保留酸度 |
     | lo5-for-010 | 三者皆於發酵中途強化，沒有任何差異 | 波特酒與雪莉酒的強化時機其實相同，只有馬德拉酒的做法不同 |
     | lo5-for-011 | 波特酒天生帶甜與強化時機完全無關，純粹是額外添加糖分所致 | 波特酒的甜度來自特殊酵母菌株，天生無法完全代謝葡萄中的糖分 |
     | lo5-for-012 | 兩者皆採相同的氧化陳年方式，沒有差異 | 兩者皆採用Flor生物陳年，差異只在於陳放時間長短 |

     （標＊的7題是DECISIONS.md #304所述、本次對話自己新犯的錯誤，其餘35題為原始題庫既有問題）

     原因：比照#301逐題對照表的既定格式，讓使用者不必重讀程式碼diff即可複核每一題的判斷是否合理；掃描後仍保留9個關鍵詞殘留未修正（如lo3-rie-013「因為它從未離開過原產地」、lo3-pn-001「天然完全不具備任何陳年潛力」等），這些選項雖含類似否定字眼，但並未直接否定題幹本身明講的事實，而是需要實際葡萄酒知識才能判斷對錯（例如需要知道Pinot Noir其實具備一定陳年潛力，而非單靠題幹文字邏輯推理），刻意不列入本次42題修正範圍，若使用者認為這9題也需一併處理可再提出。

## 2026-09-08 第三輪稽核：全題庫「全稱／唯一化斷言」型干擾項共63題（89處選項）＋「不屬於」題型格式洩題2題

310. **使用者指示全面掃描300題題庫，找「沒有鑑別度」的選項（不需葡萄酒知識、純靠邏輯或語感就能刪去）以及用字通順度問題，發現新模式**：寫node腳本以「只／只有／只會／只能／只要／純粹是／純粹取決於／絕對／毫無／唯一／永遠／完全不具備／從未／總是／所有…都／任何…都／完全沒有／完全無關／完全依賴／完全正確／必然」等把因果或範圍講死的極端斷言詞掃描全選項，統計結果：**全題庫89處這類用語，85處（95.5%）是干擾項，僅4處在正解**，且這4處正解都是「否定/讓步」語法（如「但仍受氣候…等其他因素影響」「並非只有…才能」），性質與干擾項那種「把話講死」的用法不同。判準與#306-308的「其實/反而」洩題語氣詞、#303的自我矛盾選項同屬一類問題——**不需要任何葡萄酒知識，看到選項把因果/範圍講成「只有/所有/絕對/完全」這種零例外的極端敘述，經驗上直接猜錯的命中率就很高**，等同給了不需要知識的刪去線索。涉及60題（含部分題目2個選項同時中招），另外用同一判準額外抓到`lo3-cs-014`（單寧與花香調性「必然」連動）、`lo4-gew-001`（Gewürztraminer字首「必然」是甜酒）2題，以及本輪順帶處理的長度離群題`lo3-svb-012`（正解56字、干擾項僅17-32字，且干擾項用「都極高，皆適合」這種全稱式口吻），共63題、89處選項全數改為語氣中性但知識點仍錯誤的敘述（如「只會延後採收時間」→「會延後採收時間」、「所有加美酒款都不具備任何陳年潛力可言」→「多數加美酒款陳年潛力有限」），只置換選項文字本身，不改變其錯誤/正確性質、不改變correctIndex。
     原因：`lo3-rie-013`、`lo3-pn-001`這2題正是#309(即上一段)明講「刻意排除」的題目——但排除的理由是「未直接否定題幹本身、需要實際知識才能判斷」，屬於**自我矛盾模式**的排除標準；本輪的**全稱斷言詞模式**是另一條獨立的洩題管道（干擾項用「從未」「完全不具備」這種零例外用詞，不需要知道題幹在問什麼、也不需要判斷是否矛盾，光憑「這句話講太滿」的語感統計規律就有很高機率猜對），兩者判準不同、互不衝突，因此這2題在被前一輪正確排除後，於本輪基於新判準重新收錄，如實記錄避免未來誤以為是重複勞動或判斷反覆。
311. **同時發現「下列哪一個產區不屬於XX代表產區」題型中，2題的正解選項是唯一缺少地名後綴的選項**：`lo3-rie-007`（其餘3個選項都有「地區」後綴，正解`Chianti Classico（經典奇揚地）`沒有）、`lo3-syr-008`（其餘3個選項都有「地區/產區/一帶」後綴，正解`Rioja（里奧哈）`沒有）——考生純看格式差異就能挑出正解，不需要任何產區知識。修正方式為替正解補上與其他選項一致的「產區」後綴（如`Rioja（里奧哈）產區`），不變更選項實質內容。全題庫其餘8題同類型「不屬於」題經逐題檢查後4個選項格式一致，無此問題。
312. **本輪收尾驗收**：node腳本讀入完整300題陣列——VM沙盒解析語法通過；300題結構檢查（4選項、無重複）全數通過；300題id全數唯一；correctIndex分佈維持75/76/72/77（本輪只置換選項文字，未變動任何correctIndex）；連續3題以上同正解位置殘留數為0；重新掃描本輪判準關鍵詞，干擾項殘留數為0；「其實/反而」殘留數維持0（確認未在本輪修正中意外復發）；無選項重複、無標點/贅字/重複字殘留（另見#313說明本輪同時執行的通順度掃描結果）。
313. **通順度／贅字/錯字掃描結果：全題庫目前乾淨，未發現新案例**：以正則掃描選項重複、雙字重複（如「的的」「了了」）、全形/半形標點混用、選項開頭誤植標點、explanation缺結尾標點、英數字與中文緊鄰無斷詞等模式，全部掃描結果為0，代表上個session修正的「之高」型殘留錯字已徹底清除，本輪也未產生新的贅字/錯字問題。另掃描「明顯」一詞（46處中干擾項26次、正解7次左右）逐條複核後判斷屬正常品飲強度描述用語（如「礦石感明顯」），與「其實/反而」這類純語氣連接詞性質不同，不建議處理。
314. **同判準（#310）追加掃描「完全不／全面／一概／徹底」等變體極端斷言詞，19處全數為干擾項、0處正解，其中12題與#310同一性質（把某個具體特性講死成「完全不X」），修正方式與#310一致**（如`lo3-rie-001`「完全不適合任何形式的長期瓶中陳年」→「不適合長期瓶中陳年，建議儘早飲用」、`lo3-syr-012`「Syrah/Shiraz被認為完全不含單寧成分」→「Syrah/Shiraz被認為單寧含量極低，幾乎不明顯」）；**另外6處（`lo3-pgg-001`、`lo3-pgg-005`、`lo3-pgg-012`、`lo3-syr-006`、`lo4-alb-002`×2）刻意保留未處理**——這6處的「完全不同的品種」描述，其錯誤性質來自這些品種對本身就是WSET教材點名的「同物異名」考點（Pinot Grigio＝Pinot Gris、Syrah＝Shiraz、Albariño＝Alvarinho），必須具備該項知識才能判斷「完全不同」是錯的，不像#310那批「把範圍/因果講死」的空洞斷言可以純靠語感猜中，性質不同，先列出回報使用者是否仍要處理、或维持現狀。
     原因：區分「詞面絕對化，但論述內容仍需要知識才能判斷對錯」與「詞面絕對化到單靠語感統計就能猜對」這兩種情況，避免把#310確立的判準無限上綱到所有含「完全不」的選項，誤刪本身就需要知識支撐的合理干擾項。

## 2026-09-08 第四輪：使用者實測發現「題幹已明講品種特性/聲譽，選項卻直接否定」的新一波自我矛盾漏網題，共5題

315. **使用者截圖回報`lo3-cha-014`（Chardonnay全球適應力最強）4選項中有3個都直接違背題幹已預設的「適應力最強」前提**（「全世界僅有單一產區能夠種植」「只適合單一固定氣候」「完全不需要陽光與水分」），其中「單一產區」「單一固定氣候」兩項屬於上個session的#303-309判準（題幹預設事實、選項卻直接否定）本應被抓到卻漏網，「不需要陽光與水分」則是不需任何葡萄酒知識、純靠基礎常識就能判斷不可能的選項——四選一實質上已被簡化成幾乎是唯一合理選項，這題示範了本session#310建立的「全稱斷言詞」判準與舊有的「自我矛盾」判準即使都跑過，仍可能因為干擾項沒有使用任何被掃描到的關鍵詞（如「僅有」不在#310詞庫、「單一」不在#303-309詞庫）而漏網。3個選項全數重寫為需要真正Chardonnay知識才能排除的錯誤歸因（如誤植抗病力、誤植品種香氣強度、誤植糖酸穩定性為適應力成因），不再與題幹「適應力最強」的方向直接衝突。
     原因：如實記錄「本session自己編輯過這題其中2個選項、卻沒發現第三個未改選項與整題結構仍有問題」——第4-5節提到的軟化只處理了關鍵詞層級（只能→主要適合），沒有回頭檢查整題4選項是否仍有「與題幹方向相反」的深層結構問題，是本次稽核方法論的侷限，提醒未來同一原則需要「逐題整體檢視」而非只逐選項比對關鍵詞庫。
316. **依同一判準（題幹已用「聞名／著稱／公認」等詞明講品種的招牌特性方向，選項卻直接主張相反方向）反向排查全題庫，找到含此類措辭的21個題幹，逐題複核後另外抓到4題同類問題**：`lo4-neb-004`（題幹明講Nebbiolo以「陳年潛力聞名」，選項卻寫「不需要瓶中陳年，最佳風味在裝瓶當下」）、`lo3-cs-010`（題幹明講CS單寧「以緊實厚重聞名」，選項卻寫「口感最為柔和圓潤」，且另一選項「單寧含量與果皮厚度完全無直接關聯」屬於#310判準的「完全無...關聯」變體、未被原關鍵詞庫`/完全無關/`的精確比對抓到）、`lo3-mer-014`（題幹已明講「即使單寧總量不算最低」，選項卻寫「幾乎不含任何單寧成分」直接否定題幹前提）、`lo4-zin-003`（題幹明講Zinfandel「常以偏高酒精濃度聞名」，2個選項分別寫「只出現在少數酒莊頂級款，並非普遍常見特性」與「多半釀成低酒精度的清爽風格」，雙雙直接否定題幹前提）。全數改為維持題幹既定方向、但用錯誤的具體成因（如錯誤的品種對比、錯誤的釀造技術歸因）作為干擾項，需要實際知識才能排除。
     原因：`lo4-neb-004`的問題選項正是本session#310自己編輯過的成果（僅軟化「完全不需要」→「不需要」，未發現整句仍與題幹「聞名」方向矛盾），與#315同一教訓；`lo3-cs-010`的「完全無直接關聯」變體證實純關鍵詞比對法有天生的覆蓋率上限，未來若再稽核同類問題，應以「題幹是否已預設某個方向性事實」為主要判準去逐題人工複核，關鍵詞掃描只能當作縮小範圍的輔助手段，不能取代人工判斷。
317. **本輪收尾驗收**：node腳本重新讀入完整300題陣列——結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；重新掃描#310-314全部判準關鍵詞，殘留數仍為0（僅剩#314記錄的6處「同物異名」已知例外+1處誤判「一概而論」為假陽性，皆維持原判斷未變動）；額外以「聞名/著稱/公認/以…知名/素有」等詞掃描全題幹找出21題，逐題人工複核（非僅關鍵詞比對）確認僅5題（`lo3-cha-014`、`lo4-neb-004`、`lo3-cs-010`、`lo3-mer-014`、`lo4-zin-003`）有選項與題幹方向直接矛盾，其餘16題選項雖涉及品種特性比較，但錯誤方向皆需要實際知識才能判斷，非純邏輯可刪去。

## 2026-09-08 第五輪：處理#314保留的6處「同物異名」考點選項

318. **使用者複核#314列出的6處清單後，選擇處理方向：保留「這兩者其實是不同東西」的錯誤方向，但換成更具體、需要對照產區/釀造/克隆品系細節才能判斷的錯誤敘述，不再使用可被單純語感刪去的「完全不同的品種」空泛講法**。5題6處全數修改：
     - `lo3-pgg-001`：「兩個完全不同的品種，只是名字相似而已」→「長期地理隔離下各自演化的近緣品種，基因已有明顯差異」
     - `lo3-pgg-005`：「兩者風格差異純粹來自完全不同的基因型」→「兩者風味差異源自不同的基因表現，義式版本為天生低酸的突變品系」
     - `lo3-pgg-012`：「因為兩者是完全不同的品種，陳年潛力天生不同」→「因為兩地採用的克隆品系（clone）不同，義大利選育的克隆天生早熟早飲、缺乏陳年潛力」
     - `lo3-syr-006`：「是兩個完全不同的獨立品種，僅風味相近」→「是法國原生種與澳洲當地演化出的近緣突變種，基因已有分化」
     - `lo4-alb-002`（2選項）：「稱為Alvarinho，兩者是完全不同的獨立品種」→「稱為Alvarinho，是Albariño與當地野生品種雜交後產生的獨立品種」；「名稱雖相同，但兩地種植的是基因上完全不同的變異品系」→「名稱雖相同，但兩地種植的是經數百年分別演化、基因已明顯分化的變異品系」

     每處改寫都保留「兩者其實不是同一品種」的錯誤方向與難度定位，但把理由換成具體的（偽）技術性描述（地理隔離演化、基因表現、克隆品系差異、雜交來源），考生必須知道WSET教材明講的「這幾組其實是同物異名」才能排除，不能單純靠句型模式（重複出現「完全不同」）跨題累積經驗猜題。
     原因：呼應#314討論時使用者提出的疑慮——即使錯誤方向對單一題目而言需要知識才能判斷，但「同一種題型固定用同一句『完全不同的品種』當萬用干擾項」本身在跨題重複曝光下仍可能形成可被記憶的模式，因此改寫時特別讓每一題、每一處的具體錯誤敘述互不相同，避免重蹈覆轍。
319. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；重新掃描#310-314判準關鍵詞，全題庫僅剩1處已知假陽性（`lo3-cha-010`「無法一概而論」，屬正常語意保留判斷句，非萬用刪去型干擾項），其餘全數清除，含本輪處理的6處。

## 2026-09-08 第六輪：使用者截圖回報question欄位出現「confusionNote」字樣，全題庫排查資料庫欄位名稱洩漏

320. **使用者截圖回報`lo3-pgg-006`的題幹出現「confusionNote建議消費者應以什麼作為判斷風格的依據？」，追問這是什麼意思**：確認`confusionNote`是`data/wine-data.js`裡每個品種資料物件內部的實際欄位名稱（用來記錄該品種常見的混淆說明），出題時應轉寫成通順中文敘述，但這題直接把程式碼欄位名稱原封不動貼進題幹，導致讀者看到一段無意義的英文駝峰命名字串插在句子中間。使用者接著要求全面排查是否還有其他同類洩漏。
321. **以正則掃描`data/quiz-data.js`全文中「小寫開頭＋內含大寫」的駝峰式字串，比對`data/wine-data.js`實際定義的所有欄位名稱**，確認洩漏的欄位共8個：`aromaWheel`（18處）、`confusionNote`（11處）、`representativeRegions`（8處）、`servingTemp`（9處）、`foodPairingTags`（8處）、`skinColor`（8處）、`originCountry`（8處）、`styleSummary`（10處），合計73處，全數集中在LO3每個品種固定的第13-14題「品種資料卡」子系列（風格摘要／混淆對比／香氣輪／代表產區／侍酒溫度／搭餐建議／果皮顏色與原生國）——研判是這批題目當初由`wine-data.js`資料物件半自動轉寫而成，轉寫時忘記把欄位名稱替換成中文敘述，是系統性的出題腳本疏漏，非個別題目的偶發錯字。其中9處位於`question`欄位（直接影響作答前的閱讀，含`lo3-pgg-006`與`lo3-cha-013`，其餘7處為「品種的aromaWheel香氣輪列出哪些核心香氣？」句型），其餘64處位於`explanation`欄位（作答後才顯示，但同樣會誤導學習者以為欄位名稱是某種專有名詞）。
     原因：`explanation`欄位確認會顯示給使用者（`js/quiz.js:630`「解析：${q.explanation}」），並非僅供內部除錯用的隱藏欄位，因此這73處全部需要修正，非僅question欄位優先。
322. **修正原則：依欄位語意換成對應中文敘述，不改變題目測驗的知識點**——`aromaWheel`→「香氣輪」（WSET官方品飲詞彙本身的中文譯名，直接替換即可，很多句子後面已經接了「香氣輪」三字形成「aromaWheel香氣輪」重複，直接刪除英文部分）；`confusionNote`／`styleSummary`→「官方規格」（比照全題庫既有27處「官方規格」的慣用寫法，保持用語一致，不另創新詞）；`representativeRegions`→「代表產區」；`servingTemp欄位標示為`→「建議侍酒溫度為」；`foodPairingTags列出`→「建議搭餐項目包括」；`skinColor:'white'/'red'、originCountry:'France(法國)'`這種直接外露的`key:'value'`程式碼語法（8處）→改寫成完整中文句子（如「果皮顏色為白葡萄，原生國為法國。」）。
323. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變，本輪未改動任何選項與correctIndex，只改question/explanation敘述文字)、no-run規則全數通過；全文重新掃描駝峰式字串，僅剩quiz-data.js本身合法的schema欄位（`sourceType`/`sourceId`/`correctIndex`）3組，8個洩漏欄位名稱共73處全數清除為0；抽查修正後的題幹與解析語句通順、不影響原本測驗的知識點。

## 2026-09-08 第七輪：使用者截圖回報`lo6-006`漏網的「完全無法」變體絕對化干擾項

324. **使用者截圖回報`lo6-006`（Coravin惰性氣體保存系統題）選項「完全無法延長酒款的賞味期限之久」也是可被邏輯直接刪去的干擾項**——跟本session第一輪（#310-314）處理過的「完全不」同一詞族，但用「無法」而非「不」，沒被當時的正則`/完全不/`等關鍵詞覆蓋到而漏網。以「完全無法／完全無／全無」等新關鍵詞反向排查全題庫，另外找到2題同類問題：`lo3-svb-007`、`lo3-rie-004`（皆為「兩品種比較，題幹問關鍵差異，選項卻寫『兩者風味特徵幾乎完全無法區分辨別』」，屬於上個session #306-307已處理過的「萬用比較題干擾項」同一種問題，只是換了「完全無法」這個措辭又漏網一次）。3題全數改為需要實際知識才能判斷的具體錯誤敘述：
     - `lo6-006`：「完全無法延長酒款的賞味期限之久」→「延長效果與軟木塞新舊有關，通常不到1天」
     - `lo3-svb-007`：「兩者風味特徵幾乎完全無法區分辨別」→「兩者主要差異來自產區氣候而非品種本身，本質風味相近」
     - `lo3-rie-004`：「兩者風味特徵幾乎完全無法區分辨別出來」→「兩者風味差異主要受釀造溫度影響，品種本身香氣邏輯相近」
325. **使用者同時提出`lo6-006`正解選項「延長數週甚至數月，全程不需拔出軟木塞」本身橫跨週到月兩個單位、宣稱幅度明顯大於其餘3個選項（1-3天/約1週/不到1天），觀感上像是「最誇張的選項就是正解」的視覺提示**。經確認這是Coravin氬氣保存系統的真實官方效果（業界與WSET教材公認事實，非誇大杜撰），且該題4個選項本來就對應4種不同保存技術、真實效果落差本來就很大，並非人為刻意誇大單一選項製造漏洞。與使用者討論後**維持現狀不修改**，避免為了降低視覺提示而犧牲敘述的事實精確度。
     原因：區分「選項用詞本身邏輯上必然為假（如完全無法/完全不）」與「選項描述的事實本身就是同類技術中最突出的一個、只是恰好也是正解」這兩種情況——前者是可修正的敘述缺陷，後者是真實世界事實分佈不均使然，若為了消除視覺落差而弱化正解的精確度，反而會犧牲內容正確性，不符合本專案內容正確性優先於敘述均質化的原則。
326. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；重新掃描「完全無法／完全無／全無」等新關鍵詞及本session全部既有判準關鍵詞，全題庫僅剩1處已知假陽性（`lo3-cha-010`「無法一概而論」）維持不變，其餘全數清除為0。

## 2026-09-08 第八輪：使用者發現`lo6-023`／`lo6-024`一組「食物端／酒液端因子分類」題有跨類別洩題問題，重新設計題幹與選項

327. **使用者截圖回報`lo6-024`（問「何者屬於會被食物影響的葡萄酒成分」）4選項中3個（脂肪/鹹度/辣度）其實是食物才有的屬性、根本不是葡萄酒的成分，光憑常識判斷「這些詞是食物的東西不是酒的東西」就能直接鎖定正解「苦味」，不需要任何WSET知識**。追問後使用者接著指出鏡像題`lo6-023`（問「何者不屬於會影響葡萄酒的食物成分」）也是同一種問題：3個選項（辣度/鮮味/甜度）是食物屬性，正解「單寧」卻明顯是酒才有的詞，同樣純靠類別直覺就能作答，兩題都要處理。
328. **與使用者討論處理方案時發現：若只是把跨類別的干擾項換成「同類但正解仍唯一」的選項，兩題會出現結構性矛盾**——`lo6-024`已知的5項「酒液端因子」（甜度/酸度/苦味-單寧橡木桶/果香度/酒精）依WSET教學全部都「會被食物影響」，若4個選項全部換成酒液端詞彙、題幹仍問「何者屬於會被食物影響的酒液成分」，會變成4個選項全部正確、沒有單一正解；`lo6-023`同理。因此最終方案是**選項換成同類詞彙的同時，也把題幹改得更具體，鎖定WSET食物搭配教學中一條具體的單向交互作用規則**，讓正解在同類選項中仍然唯一：
     - `lo6-023`：題幹改為「食物中的哪一項特性最容易讓葡萄酒的單寧顯得更緊澀苦硬？」，4個選項全改為食物端詞彙（甜度/鮮味/辣度/脂肪），正解改為「鮮味（Umami）」（WSET經典教學：鮮味重的食物讓單寧感覺更粗糙苦澀甚至帶金屬感，是知名搭餐地雷）。
     - `lo6-024`：題幹改為「食物中的『脂肪』與『鹹味』最容易軟化或降低葡萄酒的哪一項成分？」，4個選項全改為酒液端詞彙（甜度/酸度/酒精濃度/苦味），正解維持「苦味（來自單寧或橡木桶）」（WSET經典教學：脂肪與鹹味會軟化單寧、降低苦澀感知）。
     兩題正解在選項陣列中的原始index（`lo6-023`維持1、`lo6-024`維持3）刻意不變，避免影響correctIndex分佈與no-run規則。
     原因：這類「哪個屬於／不屬於某分類清單」題型，只要類別本身在直覺上界線清楚（食物才有的味覺特徵 vs. 酒才有的品飲屬性），就一定會被單純的類別辨識破解，是比關鍵詞掃描能抓到的問題更根本的**題型結構性缺陷**，不是換幾個字就能解決，需要同時調整題幹讓正解回到「需要記住具體交互作用規則」而非「需要分辨兩個類別」；改寫涉及具體的WSET食物搭配教學內容（脂肪/鹹味軟化單寧、鮮味放大苦澀感這兩條規則），已向使用者說明沒有原始教材可逐字核對、僅基於一般餐酒搭配教學常識，經使用者確認方向後執行。
329. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變，兩題正解index刻意維持原位)、no-run規則全數通過；`lo6-023`與`lo6-024`兩題4個選項分別純粹為食物端/酒液端詞彙，無跨類別選項殘留。

## 2026-09-08 第九輪：新增功能——上方選單「模擬考」項目一律導回模擬考首頁

330. **使用者要求：不管目前瀏覽哪個分頁，點擊上方選單下拉的「模擬考」項目後，都要回到模擬考首頁（`quiz-state-start`，有開始測驗/選LO練習/查看歷史成績按鈕的畫面），而非停留在上次離開時的內部畫面**——原本`selectTabFromGroup()`只切換外層`.panel`可見性，不會重置模擬考分頁內部的`.quiz-state`子畫面，若使用者在測驗進行到一半時切去其他分頁、再點回模擬考，會看到原本停留的畫面（如作答中畫面、結果畫面）而非首頁。
331. **追加需求：若使用者當下有一份「尚未繳卷的測驗」正在進行，點擊此選單項目跳轉前須先跳確認視窗提醒**。討論處理方式時發現網站已有現成的「離開測驗」按鈕（`quiz-state-active`左側欄，`quizExitToStart()`，見#280）做完全一樣的事——確認提示、確認後清空進度並回首頁、取消則不做任何事。與使用者確認後**直接沿用`quizExitToStart()`既有邏輯**，不另外開發「僅切畫面、保留進度」的版本，維持「點擊視為離開此次測驗」語意一致，避免同一個「回首頁」動作在不同入口出現不同的資料保留結果讓使用者混淆。
332. **實作：新增`goToQuizHomeFromNav(itemBtn, evt)`（[js/quiz.js](js/quiz.js)，緊接在`quizExitToStart()`後）**，邏輯為：若`!quizSubmitted && quizQuestions.length > 0`（有未繳卷的測驗），呼叫`quizExitToStart()`；呼叫後若`quizQuestions.length`仍大於0（代表使用者在確認視窗按下取消），直接`return`不做任何分頁切換；否則（本來就沒有進行中的測驗，或使用者已確認離開）才呼叫`selectTabFromGroup(itemBtn,'quiz',evt)`切換外層分頁並呼叫`showQuizState('start')`確保顯示首頁。[index.html](index.html)第63行選單項目的`onclick`由`selectTabFromGroup(this,'quiz',event)`改為`goToQuizHomeFromNav(this,event)`。
333. **驗證方式：Playwright啟動本機靜態伺服器實測3種情境**（因涉及下拉選單點擊、confirm對話框、跨分頁狀態，純資料驗證無法涵蓋，改用瀏覽器實測）——①無進行中測驗時點選單：直接顯示首頁，無彈窗；②練習進行中、切到其他分頁後點選單：跳出「練習尚未完成，確定要離開嗎？」確認框，取消後仍停留在原本的作答畫面、外層分頁也未切換；③同情境按下確認：正確清空進度並切回模擬考首頁。三種情境結果皆符合預期。

## 2026-09-08 第十輪：LO練習/模擬考最後一題「下一題」按鈕改為「繳卷」，左側按鈕文字精簡

334. **使用者截圖回報LO篩選練習最後一題（第8/8題）時，右下角「下一題」按鈕呈現disabled灰階，只能靠左上角的「結束測驗並繳卷」按鈕交卷，不夠直覺**：修改[js/quiz.js](js/quiz.js)的`renderQuizQuestion()`，在最後一題時把`quiz-next-btn`文字改為「繳卷」、改綁`submitQuiz(false)`（不再是disabled狀態），非最後一題時維持原本文字與`quizGoToOffset(1)`行為；改用`.onclick`屬性動態指定，覆蓋原本[index.html](index.html)行內`onclick="quizGoToOffset(1)"`的靜態綁定。同時應使用者要求，把左側「結束測驗並繳卷」按鈕文字精簡為「結束測驗」（`quiz-submit-btn`的`onclick`與功能不變，僅改文字）。
335. **驗證方式：Playwright啟動本機靜態伺服器實測**——完成LO練習前7題後導覽至第8題，確認按鈕文字變為「繳卷」；點「上一題」回到第7題，確認按鈕文字恢復「下一題」；再前進到第8題點擊「繳卷」，確認正確觸發`submitQuiz`並顯示結果畫面；左側按鈕文字確認顯示「結束測驗」。皆符合預期。

## 2026-09-08 第十一輪：LO練習與模擬考結算頁面新增「返回首頁」按鈕

336. **使用者截圖回報LO練習做完的對答案結算頁面，只有「再練一次」「換一個LO」兩顆按鈕，沒有直覺的「返回首頁」按鈕**：修改[js/quiz.js](js/quiz.js)的`renderQuizResults()`，在practice模式的`actionsWrap`新增第三顆按鈕`<button onclick="showQuizState('start')">返回首頁</button>`，樣式比照全站既有「離開測驗」等次要動作的中性灰階風格（非burgundy強調色），與前兩顆主要/次要動作按鈕做視覺區隔。
337. **同時發現正式模擬考（exam模式）的結算頁面也只有「重新測驗」一顆按鈕、同樣缺少返回首頁的路徑，屬於同一個UX缺口，一併補上同款「返回首頁」按鈕**，維持exam與practice兩種模式結算頁面的行為一致性；因結算頁面此時`quizSubmitted`已為`true`（測驗已繳卷），直接呼叫`showQuizState('start')`即可，不需要像[js/quiz.js](js/quiz.js)的`goToQuizHomeFromNav()`那樣先跑「離開測驗」確認流程。
338. **驗證方式：Playwright實測**——practice模式完整跑完8題後點擊「返回首頁」，確認正確切回`quiz-state-start`；exam模式透過`page.evaluate`直接呼叫`submitQuiz(true)`模擬繳卷，確認結算頁面`actionsWrap`同時渲染「重新測驗」與「返回首頁」兩顆按鈕。

## 2026-09-08 第十二輪：使用者發現`lo4-sem-003`題幹已預設「誤判」方向、選項卻反向否定，擴大排查同類「誤判/迷思」框架題

339. **使用者截圖回報`lo4-sem-003`（年輕干型Sémillon評價題）題幹「容易被誤判為無特色」已預設「無特色是誤判」這個前提，但4個選項中3個（釀造失誤所致／品質低劣沒有改善可能／建議年輕時飲用完畢）都直接否定這個前提，等於這3個選項不需要任何Sémillon知識、純靠題幹邏輯就能排除，只剩1個選項可選**。這是#303-309、#315-317已建立判準的同一種問題（題幹已預設方向，選項卻直接否定），但關鍵詞是「容易被誤判為／常被誤認／常見的迷思」這類框架語句，不在先前任何一輪的正則涵蓋範圍內，屬於新的漏網變體。
340. **以「容易被誤判／容易被誤解／常被誤認／容易被誤以為／誤判為／誤解為／常見的迷思／容易讓人誤判」等詞反向排查全題幹，找到5題含此類框架語句，逐題人工複核（而非僅關鍵詞比對，因為「誤判／迷思」框架不必然導致選項自我矛盾，需視個別選項是否真的直接否定前提）**，確認3題另有同類問題：
     - `lo3-rie-003`：選項「這個迷思大致正確，Riesling多半釀成甜型」直接肯定題幹已標示為「迷思」的說法，本session稽核初期(#310批次)只軟化了「完全正確」→「大致正確」，未處理結構性矛盾，本輪改為「這個迷思源自萊茵河流域法規規定Riesling裝瓶前需額外添加糖分」（需要知道real規定內容才能判斷為假）。
     - `lo4-neb-002`：選項「這個誤判是正確的，單寧確實很輕盈」與「單寧含量極低，酒體極輕盈」皆直接肯定題幹已標示為「誤判」的說法（後者是#314批次只移除「完全不含任何」，同樣未處理結構性矛盾），本輪分別改為「這個誤判源自Nebbiolo裝瓶前會刻意過濾，去除多數單寧顆粒」與「單寧含量中等，主要來自橡木桶陳年而非果皮萃取」。
     - `lo4-sem-003`：3個選項全數改為與題幹「陳年後才展現實力」前提不衝突、但需要具體Sémillon知識才能判斷為假的錯誤成因敘述（誤植採收時機、誤植發酵容器、誤植年份氣候）。
     另外2題（`lo4-alb-003`、`lo4-pin-002`）複核後選項雖涉及品種比較，但錯誤方向皆需要具體知識判斷（如香氣特徵歸屬、風味強弱對比），未直接否定題幹「確實存在差異」的前提，判斷不需修改。
     原因：這是本session第二次發現「先前已軟化關鍵詞用語的選項，其實底層結構性矛盾仍未解決」（第一次是#315-317的`lo4-neb-004`案例），再次證實「只換掉觸發正則的詞」跟「解決選項本身的邏輯問題」是兩件事，關鍵詞掃描只能縮小複查範圍，最終仍需要逐題判斷選項內容與題幹前提是否存在方向性衝突。
341. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；4題修改後選項皆不再與各自題幹的「誤判/迷思」前提方向衝突。
342. **使用者複核`lo4-neb-002`時發現#340新寫的選項1「這個誤判源自Nebbiolo裝瓶前會刻意過濾，去除多數單寧顆粒」本身邏輯不通**：若過濾真的去除了多數單寧，那考生判斷「這款酒單寧低」就會是對的、根本不構成「誤判」，這個選項的因果關係是自我瓦解的，且答非所問（題目問的是「外觀顏色與單寧/酒體的關聯性」，這個選項講的是與外觀無關的裝瓶前處理工法），已改為「顏色偏淡是因為果皮特別薄，因此單寧萃取量也偏低」——維持在「外觀→單寧」這條題目實際在問的邏輯鏈上，且錯誤點（Nebbiolo果皮並不特別薄）需要具體品種知識才能判斷。
     原因：構造干擾項時只顧著「避免直接否定題幹前提」，沒有反過來檢查「這個新編的錯誤原因本身是否讓誤判失去意義、或是否還在回答題目實際問的東西」，是新的自我檢查盲點，記錄下來提醒未來編寫替代選項時，除了檢查邏輯方向，也要檢查因果鏈是否自洽、是否切題。
343. **使用者進一步指出`lo4-neb-002`選項2「顏色深淺與單寧含量呈正相關，顏色淺單寧低」也該修**：原本認為這是「考通則的例外」（一般認知顏色深淺與單寧相關，但Nebbiolo是例外），判斷不需要修改；但使用者指出考生根本不需要知道這個通則是否普遍成立，只要把這句話套用在題目問的Nebbiolo身上，結論就是「顏色淺→單寧低」，跟題幹已經明講是誤判的說法一樣，一樣能純靠邏輯刪去。**藉此修正了整個判準的表述方式：不是看選項是「直接斷言」還是「包裝成通則」，而是看選項套用到題目問的具體對象後，結論是否等於被題幹判定為誤判/迷思的那個結論，不管中間繞了幾層推論都算**。已改為「單寧與酸度接近中等水準，介於黑皮諾與卡本內蘇維濃之間」（錯誤的具體量級比較，而非重申「淺色→低單寧」的結論）。
     原因：用修正後的框架回頭複查同一輪`lo4-alb-003`、`lo4-pin-002`兩題（皆維持不動的判斷），確認這兩題的「誤判/誤認」講的是「容易誤以為兩者相近」，但4個選項全都承認「確實有差異」，沒有任何選項套用後會導出「其實沒有差異」，跟`lo4-neb-002`的情況不同，故判斷維持不變；另外以更廣的同義詞（普遍認為/一般人以為/許多人以為/坊間流傳/常見錯誤認知等）重新掃描全題庫題幹，確認符合「誤判/迷思」框架的題目仍是同一批5題，沒有新的漏網候選，本輪排查完整。
344. **本輪追加收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過。

## 2026-09-09 第十三輪：`lo1-029`（酒標Botrytis/Noble Rot標示題）3個選項答非所問／邏輯矛盾

345. **使用者截圖回報`lo1-029`（酒標上「Botrytis/Noble Rot」標示所指為何）4選項中3個都有問題，且指出的都不是「詞面太絕對」這類本session已知的模式，而是更根本的缺陷**：「主要出現在紅葡萄品種上，白葡萄較少感染」答非所問（題目問標示代表什麼意思，這個選項在講感染的品種偏好）；「一種葡萄病害，感染後葡萄通常會被淘汰不用」邏輯自我矛盾（若葡萄真的通常被淘汰，就不會有這瓶酒、也不會有這個標示可看）；「專指葡萄藤本身隨樹齡老化的自然現象」則是完全不相關的另一個概念（葡萄藤老化與貴腐黴無關），且酒標本來就不會有這種標示方式。這3個選項都是本session稽核初期（#310批次）只做了「拿掉絕對化用詞」的表面修改（只會→主要、必須全數→通常），沒有回頭檢查選項是否真的在回答題目、或選項本身敘述是否自洽，與`lo4-neb-002`（#342）同一教訓的第三次重演。
346. **與使用者確認方向後，3個選項全數改為WSET甜酒教材中真正會與「貴腐黴/Noble Rot」放在一起教、容易混淆的其他甜酒濃縮工法，讓4個選項都是「這個標示可能代表哪種甜酒濃縮工法」的合理候選答案**：
     - 「主要出現在紅葡萄品種上，白葡萄較少感染」→「同一種黴菌若在潮濕天氣下大量繁殖未及時轉為貴腐，會演變成必須淘汰的灰黴病」（灰黴病與貴腐黴是同一種真菌Botrytis cinerea在不同天候下的兩種樣貌，是WSET教材會特別對比的知識點）
     - 「一種葡萄病害，感染後葡萄通常會被淘汰不用」→「指葡萄採收後刻意風乾以提升糖度的傳統工法」（風乾濃縮法/Appassimento，另一種真實的甜酒濃縮技術）
     - 「專指葡萄藤本身隨樹齡老化的自然現象」→「指葡萄成熟後刻意延後採收、讓糖分持續累積的晚摘工法」（晚摘法/Vendange Tardive，也是常與貴腐黴一起比較的甜酒技術）
     只置換選項文字，題幹、正解與correctIndex皆未變動。
     原因：這是「換詞不等於解決結構問題」教訓第三次出現，補充掃描全題庫「酒標上...標示所指為何」同類題型，僅另找到`lo1-030`（Vintage標示）一題，複查後3個選項本身就是合理的候選答案（誤認為限定氣泡酒／誤認為陳放年數／誤認為酒莊創立年），無同類問題，不需修改。
347. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；`lo1-029`4個選項確認皆為「標示代表哪種甜酒工法」的合理候選、無答非所問或自我矛盾。

## 2026-09-09 第十四輪：白葡萄品種題目中誤植「單寧厚重」選項，牴觸更早期LO2基礎知識

348. **使用者截圖回報`lo3-svb-001`（Sauvignon Blanc經典風格特徵）與`lo3-cha-010`（Chardonnay為何幾乎不帶單寧口感）2題選項中出現「單寧明顯厚重」「單寧是Chardonnay最主要的風味特徵」，指出這是新的一種可被秒殺的模式**：白葡萄酒發酵時幾乎不接觸果皮、單寧極低，是WSET LO2就教過的基礎知識，不需要任何特定品種的LO3/LO4知識就能判斷「白葡萄品種選項寫單寧厚重」必然是錯的；`lo3-cha-010`的案例則同時符合本session已知的「題幹已預設事實、選項卻反向否定」判準（題幹已明講「幾乎不帶單寧口感」，選項卻說單寧是主要特徵）。
349. **以`data/wine-data.js`中`skinColor:'white'`的16個白葡萄品種id為範圍，全面掃描`data/quiz-data.js`中對應`sourceId`的題目選項是否提及「單寧」，找到12處，逐一複核後確認7題8處屬於「牴觸白酒無單寧基礎知識」的問題選項**：`lo3-cha-003`、`lo3-cha-004`（冷涼/溫暖氣候Chardonnay風格題）、`lo3-svb-014`（Sauvignon Blanc香氣輪，其中一個候選寫「結構化單寧」）、`lo3-rie-001`（Riesling鮮明特徵）、`lo3-rie-011`（Riesling陳年潛力成因）、`lo3-rie-014`（Riesling單寧與酒體表現，2處）、`lo4-ver-002`（Verdicchio陳年潛力成因）。全數改為不涉及單寧、但仍需要具體品種知識才能排除的錯誤風格/成因敘述（如誤植甜度方向、誤植香氣調性、誤植防腐機制、誤植果皮厚度）。另外4處複核後判斷不需修改：`lo3-cha-010`原有的2處（果皮薄／區域浮動，皆為合理的錯誤成因，非「單寧厚重」式的基礎知識牴觸）與`lo4-vio-002`（該題「單寧」指的是北隆河紅酒混調對象Syrah本身的單寧，並非在描述Viognier這個白葡萄品種自身的單寧特性，性質不同）。
     原因：這是本session第一次針對「牴觸更早期LO（LO2）已教過的基礎知識、而非牴觸本題題幹或本題所測驗的品種知識」這個新維度做全面排查，跟先前#310-347各輪「題幹自我矛盾」「全稱斷言詞」「跨類別答非所問」等既有判準是互補、不重疊的獨立面向，記錄下來作為未來若再稽核時的第4種判準類型。
350. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過；以白葡萄品種`sourceId`為範圍重新掃描「單寧」提及處，剩餘6處皆為已複核判斷合理保留的選項。

## 2026-09-09 第十五輪：`lo6-004`儲存濕度題——自我矛盾選項＋極端值製造「中庸正解」猜題捷徑

351. **使用者截圖回報`lo6-004`（建議的葡萄酒儲存濕度區間）有雙重問題**：①「濕度沒有影響」跟題幹「建議的...濕度區間為何」已預設濕度確實有影響、且有建議區間的前提矛盾，屬於本session已知判準；②即使拿掉①，剩下的`20–40%`與`90–100%`兩個數字選項本身就是不需要任何葡萄酒知識、單憑常識就顯得誇張的極端值（近乎沙漠乾燥／完全飽和結露），會讓正解`50–80%`因為「看起來是唯一中庸合理的選項」而被用跟本題知識點無關的猜題捷徑選中，這是本session第一次發現的**「極端值夾正解」型鑑別度問題**，不同於先前任何一種已知模式（全稱斷言詞、自我矛盾、答非所問、牴觸基礎知識）。
352. **經使用者確認後，4個選項全數改為彼此相鄰、量級接近、皆「聽起來合理」的濕度區間**（`25–40%`／`40–50%`／`50–80%`〔正解不變〕／`82–95%`），不再有一眼看穿的極端值，需要記得WSET建議的確切數字（50-80%）才能選對；正解仍在原本的array index，不影響correctIndex分佈。補充掃描全題庫其他「純數字區間選項」題目（侍酒溫度系列等），確認皆從同一套合理的溫度階梯抽取相鄰區間、正解在不同題目間位置分散，無同類「極端值夾中庸正解」問題，本輪排查完整。
     原因：如實記錄這是繼「全稱斷言詞」「自我矛盾」「答非所問/邏輯不通」「牴觸基礎知識」之後，本session發現的第5種獨立鑑別度問題類型，提醒未來若再稽核選擇題品質，除了檢查文字敘述本身，也要檢查「數值型選項的離散分布是否會透露答案方向」。
353. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過。

## 2026-09-09 第十六輪：`lo6-001`兩處選項只軟化用詞、結論仍與題幹矛盾（第4次同教訓）

354. **使用者截圖回報`lo6-001`（溫度穩定性比精確數值重要）選項「溫度波動不構成影響」直接否定題幹前提**：查證後確認這是本session第一輪（#310批次）editing過的選項，當時只刪掉了「波動無妨」，但句尾留下的「溫度波動不構成影響」實質意思不變，仍與題幹「穩定性比精確數值更重要」（預設波動確實有影響）矛盾；同題另一選項「20°C以下的溫度高低變化，對熟成沒有影響」也是同樣問題，先前用寬鬆關鍵詞（`不影響`）掃描時被誤判為跟白酒/紅酒那個合理的部分效果選項同類、一起放過，這次逐一複核才抓出來。這是「換詞不等於解決結構問題」教訓第4次出現（前3次為`lo4-neb-004`、`lo3-rie-003`/`lo4-neb-002`、`lo1-029`）。
355. **2處選項改為與題幹「穩定性vs精確數值」主題相關、但屬於錯誤優先順序判斷的說法（而非直接否定波動有影響）**：「溫度波動不構成影響」→「只要平均溫度落在建議範圍內，短期劇烈波動可以忽略不計」；「對熟成沒有影響」→「只要控制在精確的溫度數值，就不需要在意是否出現波動」。兩者都承認「波動」這件事本身存在，只是錯誤地認為在特定條件下（平均值合格／數值夠精確）就不需要在意，符合題幹本身「穩定性 vs 精確數值」的對比主題，不再是空泛的「沒有影響」。
     原因：用更嚴格的關鍵詞組（含「不構成影響」等先前遺漏的變體）重新掃描全題庫「沒有影響/不影響/沒有關係/無妨/可以忽略」等說法，逐一複核（而非像上次一樣直接歸類放過）確認除本題2處外，`lo6-020`、`lo5-spk-007`、`lo6-001`原有的「白酒/紅酒」選項皆為合理的「特定面向受影響、另一面向不受影響」具體敘述，非空泛否定，不需修改。
356. **本輪收尾驗收**：300題結構檢查、id唯一性、correctIndex分佈(75/76/72/77不變)、no-run規則全數通過。

## 2026-09-09 題庫擴充第1批：新增LO1 10題＋LO2 8題（300→318題）

357. **使用者提供出題規則草案（依本session #310-356發現的問題模式整理，涵蓋全稱斷言詞、題幹自我矛盾、答非所問、牴觸基礎知識、數值極端值、正解特別突出、分類格式一致、語氣詞、資料庫欄位洩漏共9條規則）供未來出題對話遵守，並提供18題草稿供本次對話依規則複核**：複核後僅1題（`lo2-029`低溫泡皮）違反規則2——題幹括號已明講「pre-fermentation maceration」（發酵前），但其中一個選項卻寫「多半用於已完成發酵的酒款進行二次泡皮」，直接否定題幹自己標註的時間點；其餘17題經逐題核對規則1-9皆無違規，另有3處提出但不算違規的優化建議（冰雹/水泥蛋/坡向坡度題型的干擾項說服力可再加強，供使用者參考）。使用者確認修正`lo2-029`後，交付本次插入任務。
358. **執行前哨兩句話回報「插入`data/quiz-data.js`：LO1新增10題(lo1-031~040)插入`lo1-030`後、LO2新增8題(lo2-025~032)插入`lo2-024`後，共新增18題」，經使用者「確認」後動工**。以node腳本定位既有題目`id`後的區塊結束標記（`\r\n  },\r\n`，注意本檔案為CRLF換行，純LF比對會找不到插入點），將18題物件依現有格式（單引號字串；含撇號的字串改用雙引號，如`lo1-038`解析文字中的`Appellation d'Origine Contrôlée`）插入兩個指定位置，不觸動其餘300題。
359. **收尾驗收時發現使用者提供的correctIndex序列在插入後產生1處no-run違規**：`lo1-040`＝0、緊接既有的`lo2-001`＝0、`lo2-002`＝0，形成連續3題同答案位置，這是使用者當初只核對了單一銜接點（`lo1-030`=3與`lo2-001`=0之間）、未繼續核對到`lo2-002`所致。**修正方式：僅重新排列`lo1-040`4個選項在陣列中的順序，將正解從index 0移到index 2，選項文字與correctIndex所代表的答案內容完全不變，只改變其在陣列中的位置**，重新驗證後no-run規則通過（max run降回2）。
360. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；318題全數4選項、無重複選項；
     - id唯一性：318個id全數唯一；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（修正`lo1-040`後）；
     - correctIndex分佈：0/1/2/3 = 79/81/77/81（新增18題後的最終分佈，較插入前的75/76/72/77更新）；
     - 新增18題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏）：殘留數為0。

## 2026-09-09 題庫擴充第2批：新增LO3白葡萄品種19題（318→337題）

361. **使用者交付第2批19題：Chardonnay 5題、Sauvignon Blanc 5題、Riesling 5題、Pinot Gris 4題，分4處插入既有題庫**（`lo3-cha-014`後、`lo3-svb-014`後、`lo3-rie-014`後、`lo3-pgg-014`後）。執行前哨回報時發現prompt裡「300+19=319」的預期總數未計入第1批已插入的18題，實際應為318+19=337，經使用者確認後動工。
362. **沿用第1批建立的插入方法**：node腳本以`id: '<既有題id>'`後第一個`\r\n  },\r\n`區塊結束標記定位插入點（CRLF換行），依現有格式（單引號；含撇號字串改雙引號）產生新題文字後插入，4處依序插入不互相干擾（各自的定位id在插入前皆已存在於檔案中）。
363. **收尾驗收再度發現1處no-run違規，與第1批`lo1-040`同一根因（使用者只核對單一銜接點、未繼續核對到下一題）**：新插入的`lo3-pgg-015`＝1，與既有`lo3-pgg-013`＝1、`lo3-pgg-014`＝1形成連續3題同答案位置。修正方式與第1批一致：僅重排`lo3-pgg-015`的4個選項順序（正解從index 1移到index 0），選項文字與答案內容不變，重新驗證後no-run規則通過。
     原因：連續兩批都出現「只核對插入題目與既有題目的單一銜接點、未繼續核對到緊鄰第二題」的同一種疏漏，這是使用者端correctIndex生成流程的系統性侷限，非隨機個案，建議下一批插入前的no-run自我檢查範圍至少涵蓋銜接點前後各2題（而非1題），或插入後仍由Claude Code這端做最終復查（如本次做法）作為最後防線。
364. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；337題全數4選項、無重複選項；
     - id唯一性：337個id全數唯一；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（修正`lo3-pgg-015`後）；
     - correctIndex分佈：0/1/2/3 = 87/84/81/85（新增19題後的最終分佈，較插入前的79/81/77/81更新）；
     - 新增19題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏、萬用比較干擾項如「幾乎相同/難以區分/容易混淆」）：殘留數為0。

## 2026-09-09 題庫擴充第3批：新增LO3紅葡萄品種19題，LO3全部38題新增完成（337→356題）

365. **使用者交付第3批19題：Cabernet Sauvignon 5題、Merlot 5題、Pinot Noir 5題、Syrah-Shiraz 4題，分4處插入既有題庫**（`lo3-cs-014`後、`lo3-mer-014`後、`lo3-pn-014`後、`lo3-syr-014`後）。執行前哨回報時同樣先更正prompt裡「319+19=338」的預期總數（未計入前兩批已新增的37題），確認實際應為337+19=356後動工。本批完成後LO3全部38題新增即告完成。
366. **記取前兩批連續出現同一種no-run疏漏的教訓，這次插入前先手動核對4處插入點銜接處前後各2-3題的correctIndex序列，確認結合新題序列後無連續3題同答案位置，才執行插入**，插入後自動化復查同樣通過（無no-run違規），是連續第3批操作以來首次插入前主動預防而非插入後才發現問題。
367. **收尾驗收的萬用比較干擾項關鍵詞掃描抓到1處殘留**：`lo3-mer-017`（美國華盛頓州Merlot氣候題）選項「緯度極低、終年高溫少變化，與加州溫暖產區氣候條件幾乎相同」——題幹問「相對加州有何特點」已預設兩地存在氣候差異，此選項卻直接主張「幾乎相同」，重申了被題幹否定的「沒有差異」結論，且「緯度極低」本身也是地理事實錯誤（華盛頓州緯度高於加州，並非低）。已改為「主要因當地降雨量遠高於加州，需仰賴大量排水設施避免葡萄園淹水」（誤植方向的具體錯誤成因，需要知道當地實際氣候型態才能排除），只改選項文字，不影響correctIndex位置。
368. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；356題全數4選項、無重複選項；
     - id唯一性：356個id全數唯一；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（插入前已預防，未產生新違規）；
     - correctIndex分佈：0/1/2/3 = 94/88/85/89（新增19題後的最終分佈，較插入前的87/84/81/85更新）；
     - 新增19題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏、萬用比較干擾項）：修正`lo3-mer-017`後殘留數為0。

## 2026-09-09 題庫擴充第4批：新增LO4前半12題，涵蓋10個品種（356→368題）

369. **使用者交付第4批12題：Tempranillo 2題、Sangiovese 1題、Nebbiolo 1題、Grenache 2題、Malbec 1題、Chenin Blanc 1題、Zinfandel-Primitivo 1題、Gamay 1題、Gewürztraminer 1題、Viognier 1題，分10處插入既有題庫**（各自對應品種現有題目末尾之後）。執行前哨回報時再次更正prompt裡的預期總數：prompt正文寫337題、括號算式「300+18+19+19」實際等於356，兩者互相矛盾，經核對repo實際狀態確認目前為356題，插入後預期總數應為356+12=368，非prompt寫的349，經使用者確認後動工。
370. **插入前主動執行兩項預防檢查**：①沿用第3批做法，逐一核對10處插入點銜接處前後correctIndex序列，確認結合新題後無連續3題同答案位置；②插入前先人工複核12題內容是否符合稽核規則，抓到`lo4-vio-004`一處新型態問題——選項「Viognier對霜害完全免疫，因此不需要採取任何春霜防護措施」用「完全免疫」這類極端生物學斷言，不需要任何Viognier專屬知識、單憑常識判斷「植物不可能對天然災害完全免疫」就能刪去，且此措辭未落在既有關鍵詞規則庫的比對範圍內（規則庫比對的是「完全不X」句型，「完全免疫」屬同一詞族的新變體）。已於插入前先行改為「Viognier主要的種植風險是根瘤蚜蟲害，與採收時機或酸度管理較無直接關聯」（需要具體品種知識才能排除的錯誤成因）。
371. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；368題全數4選項、無重複選項；
     - id唯一性：368個id全數唯一；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（插入前已預防，未產生新違規，連續第2批插入前預防成功）；
     - correctIndex分佈：0/1/2/3 = 97/93/86/92（新增12題後的最終分佈，較插入前的94/88/85/89更新）；
     - 新增12題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏、萬用比較干擾項、「完全免疫」等新變體）：殘留數為0（`lo4-vio-004`已於插入前修正）。

## 2026-09-09 題庫擴充第5批：新增LO4後半12題，LO4全部24題新增完成（368→380題）

372. **使用者交付第5批12題，涵蓋12個品種各1題（Sémillon／Albariño／Barbera／Corvina／Montepulciano／Pinotage／Carménère／Verdicchio／Cortese／Garganega／Fiano／Furmint），分12處插入既有題庫**（各自對應品種現有題目末尾之後，Furmint插入在LO4結尾、LO5開始之前）。本批prompt的預期總數（368+12=380）與repo實際狀態一致，執行前哨回報時已核實無誤，經使用者確認後動工。
373. **插入前依序執行第4批建立的兩項預防檢查，這次同時抓到correctIndex銜接與選項內容兩類問題**：
     - **no-run銜接檢查**：逐一核對12處插入點前後correctIndex序列，抓到2處會形成連續3題同答案位置的違規——`lo4-ver-004`（與既有`lo4-ver-002`、`lo4-ver-003`皆為1）、`lo4-cot-004`（與既有`lo4-cot-002`、`lo4-cot-003`皆為0），皆於插入前僅重排4個選項順序修正（`lo4-ver-004`正解index 1→0；`lo4-cot-004`正解index 0→2），選項文字與答案內容不變。
     - **選項內容規則複核**：以既有關鍵詞規則庫掃描12題選項，抓到2處新違規——`lo4-sem-004`「該產區的Sémillon**一律**經長期橡木桶陳年」（「一律」屬於「所有…都」同一詞族的全稱斷言詞，規則庫先前未收錄此詞）、`lo4-ver-004`「該土壤組合**完全抑制**葡萄藤生長，因此當地產量是義大利境內最低」（邏輯自我矛盾：若生長真的被完全抑制就不會有葡萄可採收、也不會有這個產區存在，與#342`lo4-neb-002`案例同一種問題）。皆於插入前修正為需要具體品種/產區知識才能排除的錯誤成因敘述，不影響已完成的no-run修正。
     原因：第4、5兩批連續在插入前主動複核，共抓到4處問題（2處no-run、2處內容規則），證實「插入前逐一核對」比「插入後才復查」更有效率，也再次確認關鍵詞規則庫本身會隨著新一批題目不斷發現新變體（一律、完全抑制），需要持續擴充。
374. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；380題全數4選項、無重複選項；
     - id唯一性：380個id全數唯一；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（插入前已修正2處，未產生新違規）；
     - correctIndex分佈：0/1/2/3 = 102/96/89/93（新增12題後的最終分佈，較插入前的97/93/86/92更新）；
     - 新增12題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏、萬用比較干擾項、「一律/完全抑制」等新變體）：殘留數為0（插入前已修正）。
     本批完成後LO4全部24題新增即告完成。

## 2026-09-10 題庫擴充第6批（最後一批）：新增LO5氣泡酒/強化酒與LO6共20題，100題擴充計畫全部完成（380→400題）

375. **使用者交付第6批、也是最後一批20題：LO5氣泡酒6題、LO5強化酒6題、LO6 storage-decanting 2題、LO6 storage-faults 4題、LO6 food-wine-interactions 2題，分5處插入既有題庫**。執行前哨回報時數字核對無誤（380+20=400），確認後動工。
376. **插入前的id衝突檢查發現本批唯一的結構性問題：LO5強化酒新增id`lo5-for-017`~`lo5-for-022`與既有題庫直接撞號**——查證後既有`lo5-for-`已有18題（001~018，非prompt預期的16題），新編號從017開始會與既有`lo5-for-017`、`lo5-for-018`重複。先跟使用者確認後，將這6題id整批往後順延2號改為`lo5-for-019`~`lo5-for-024`，內容、選項、correctIndex完全不動，其餘4組（spk、decanting、faults、food-pairing）核對後皆無衝突。
377. **插入前的no-run模擬檢查，發現id順延後衍生1處新違規並修正**：`lo5-for-019`（順延前為`lo5-for-017`）緊接在既有`lo5-for-017=2`、`lo5-for-018=2`這組既有連續對之後，若直接插入會形成連續3題同答案位置，僅重排該題4個選項順序修正（正解index 2→0），其餘4組模擬後皆無違規。
378. **插入機制的特殊處理：LO6 food-wine-interactions這組（`lo6-030`、`lo6-031`）prompt明確指出要插入在「全題庫陣列最尾端」，而非緊接在既有`lo6-024`目前所在的陣列位置**（`lo6-024`在現有380題陣列中排序極早、緊接在`lo1-001`之前，並非陣列尾端；陣列真正的尾端在批次交付當下是`lo5-for-018`）。確認`lo5-for-018`同時也是本批次LO5強化酒新增6題（`lo5-for-019`~`024`）的插入基準點，因此改用「附加於陣列真正尾端」的插入方式（在`lo5-for-018`後先附加強化酒6題成為新的陣列尾端，其餘4組正常插入後，最後再把這2題附加到全陣列的最終尾端），而非依prompt字面上的「插入在lo6-024之後」逐字操作，避免題目被插入到陣列中段錯誤位置。
379. **本批收尾驗收完整數字**：
     - 結構檢查：VM沙盒解析語法通過；400題全數4選項、無重複選項；
     - id唯一性：400個id全數唯一（含順延後的`lo5-for-019`~`024`與既有`lo5-for-001`~`018`共24題，無重複無缺號）；
     - no-run規則：全陣列連續3題以上同correctIndex殘留數為0（插入前已修正1處，未產生新違規）；
     - correctIndex分佈：0/1/2/3 = 106/101/95/98（新增20題後的最終分佈，較插入前的102/96/89/93更新）；
     - 新增20題關鍵詞掃描（全稱斷言詞、其實/反而、資料庫欄位名稱洩漏、萬用比較干擾項）：殘留數為0；
     - 陣列真正尾端確認：`QUIZ_BANK`最後兩個元素為`lo6-030`、`lo6-031`，符合prompt要求。
     - 全題庫最終陣列長度：實際跑一次`QUIZ_BANK.length`確認為**400**，與300（原始）+18+19+19+12+12+20的算式結果一致。

380. **100題擴充計畫（6批）總結**：第1批LO1+10／LO2+8（共18題，commit `ecd0bca`）→第2批LO3白葡萄品種19題（commit `9930b93`）→第3批LO3紅葡萄品種19題、LO3全部38題完成（commit `9432967`）→第4批LO4前半12題（commit `67dab2d`）→第5批LO4後半12題、LO4全部24題完成（commit `3e00e10`）→第6批LO5氣泡酒/強化酒與LO6共20題（本次commit）。18+19+19+12+12+20=100，300題原始題庫擴充至400題全部完成。6批共同抓到的系統性問題模式：①至少4批（第2、3、4、5批）在插入前後發現correctIndex銜接處產生連續3題同答案位置的no-run違規，皆透過「僅重排選項順序、不改動選項文字與答案內容」的方式修正，不影響已完成的內容查證；②第4、5、6批插入前主動以既有關鍵詞規則庫複核新題內容，持續發現「一律」「完全抑制」「完全免疫」等規則庫先前未收錄的全稱斷言詞新變體並修正；③第6批額外發現1處id編號與既有題庫衝突、1處插入位置指示與陣列實際結構不符，皆在插入前攔截修正，未造成資料損毀。全程無任何一批需要事後修補已push的commit。

## 2026-09-10 第十七輪：`lo1-010`考的是教材呈現格式而非實質知識，新的獨立問題類型

381. **使用者截圖回報`lo1-010`（白葡萄與黑葡萄成熟過程香氣/風味變化）選項「沒什麼鑑別度，且不太知道要考什麼」**：題幹問「官方規格的描述方式為何」，4個選項圍繞在「官方有沒有分開列出白/黑葡萄的變化模式」打轉（僅列黑葡萄／分別列出／完全相同不須區分／與品種無關），考的是**教材本身的呈現格式**（有沒有分開寫），而非「白葡萄與黑葡萄成熟過程中香氣/風味實際如何變化」這個實質知識——考生不需要任何葡萄成熟度的知識，只要合理推測「教材通常會把不同東西分開寫」就能推出正解，是本session發現的第6種獨立問題類型（前5種為全稱斷言詞、自我矛盾、答非所問、牴觸基礎知識、數值極端值）。
382. **以「官方規格的描述方式/如何描述/官方規格如何」等句型反向排查全題庫，找到8題含此類框架，逐題複核後確認只有`lo1-010`一題屬於此問題**：其餘7題（`lo1-012`、`lo1-015`、`lo1-019`、`lo1-035`、`lo2-013`、`lo2-014`、`lo2-022`）雖然也用「官方規格列出的...」起頭，但問的是「清單裡實際有哪些項目／灌溉在何種情況下使用／甜酒還有哪種釀造思路」，是在考真正的知識內容，「官方規格」只是引用框架，不是考格式本身，判斷不需修改。
383. **經使用者同意後，`lo1-010`改為考實質的成熟度光譜知識**：新題幹「白葡萄與黑葡萄在成熟過程中，香氣/風味的典型變化方向為何？」，正解「白葡萄由生青／高酸的柑橘調性逐漸轉為核果、熱帶水果等成熟果香；黑葡萄則由紅色漿果轉為黑色漿果甚至蜜餞乾果調性，同時單寧與糖分成熟度提升」，3個干擾項分別為方向講反、宣稱兩者完全相同、宣稱只有糖分變化沒有香氣變化，皆需要具體成熟度知識才能排除。correctIndex位置（1）維持不變。
384. **同批複核時使用者一併確認處理`lo2-014`（官方規格列出的三種主要發酵容器類型）的既有問題**：題幹已明講「三種」，選項卻寫「僅限不鏽鋼一種容器類型」，直接與題幹自己講的數字矛盾，屬於規則2案例。改為「橡木、栗木、松木，依產區傳統選用不同木材種類」（錯誤的具體木材種類清單，仍符合「三種」的題幹框架，需要知道正確答案是不鏽鋼/水泥/橡木才能排除），correctIndex（2）維持不變。
385. **本輪收尾驗收**：400題結構檢查、id唯一性、correctIndex分佈(106/101/95/98不變，本輪只改動選項文字與題幹措辭，未變動任何correctIndex位置)、no-run規則全數通過。
     另外主動額外檢查全題庫跨題完全相同選項文字（非任務要求項目），81處全為合理的共用詞彙（如侍酒溫度區間標籤在同款模板題型間本就會重複使用），非實質題目重複，不影響交付。

306. **使用者在`lo5-spk-010`（轉注法二次發酵題）發現#303-305那輪42題修正遺漏的同類問題**：題幹「轉注法（Transfer Method）的二次發酵流程為何？」已明講轉注法有二次發酵流程，選項卻寫「轉注法完全不需要進行任何形式的二次發酵」，這正是#303建立的判準（題幹已預設事實，選項卻直接否定），但上一輪關鍵詞規則過窄（`/完全不(存在|具備)/`沒涵蓋「完全不需要」；`/完全相同.*(並無|沒有)/`要求緊接特定字眼，沒涵蓋「難以區分辨別」「經常被混淆誤認」等變體），導致這題與同類題目沒被抓到。先修正這一題（新增「與水槽法相同,直接在大型加壓槽中進行二次發酵」「透過額外注入二氧化碳氣體達到起泡效果」兩個需要實際知識才能排除的干擾項），再放寬規則重新全庫掃描。
     原因：如實記錄規則本身的技術侷限（正則表達式覆蓋範圍不足），而非題目本身判斷錯誤，避免未來查閱時誤以為#303-305的42題已是最終完整版本。
307. **放寬規則後找出56題（約65個選項）符合同一判準，經使用者確認「全部處理」後修正**：絕大多數是原始題庫既有的「兩品種比較」題型，長年固定使用「兩者風味特徵幾乎完全相同，難以區分辨別／經常被混淆誤認」當作萬用干擾項（如lo3-cs-004、lo3-mer-006、lo3-pn-006/007、lo4-tem-002/004、lo4-gre-003/004、lo4-chb-003、lo4-car-003、lo4-ver-003、lo4-cot-003、lo4-gar-003、lo5-spk-003/004/008/009、lo5-for-006/014/015/016等），只要題幹問「差異為何／區辨為何／角色互補的方式」，這句話就能純靠邏輯刪去；另有一類是題幹本身已陳述某項事實作為前提（如lo1-013「調節作用是什麼」預設有調節作用、lo3-svb-003「DNA分析確認...具有親緣關係」預設關係已確認、lo4-cor-001「獨特的風乾工法如何將Corvina轉化為Amarone」預設工法存在），選項卻直接否定該前提。修正方式與#303一致：只置換選項文字，題幹/其餘選項/correctIndex皆未變動，改用需要實際知識才能排除的具體錯誤主張（如錯誤的品種歸因、錯誤的因果解釋、錯誤的具體數字或反向配對），不再使用「完全無關/完全相同/從未…過/完全不需要」等純邏輯可刪去的空泛否定句式。逐題新舊對照因規模較大（56題）未逐一列表，完整內容請查閱commit `3c81290`的diff。
308. **修正過程中使用者另外發現：選項用語大量使用「其實」「反而」這類語氣詞，會洩漏答案方向**：全題庫掃描後，選項中含「其實」或「反而」的共113處，經逐一核對，**100%是干擾項、0%是正解**——代表考生完全不需要任何葡萄酒知識，光靠「看到選項有這兩個字就排除」這個純語感捷徑，就能穩定刪去干擾項，比#306-307的邏輯自相矛盾問題更容易被抓漏洞、也更難察覺（因為每一題單獨看都不明顯，只有跨題統計才會發現這個規律）。經使用者同意後，涉及70題、113個選項全數處理：寫一支node腳本比對每處「其實/反而」的上下文，僅刪除語氣詞本身或做最小幅度的語句調整（如「反而較清爽」→「較清爽」、「其實酸度中等」→「酸度中等」），不改變選項要測驗的知識點、不改變其錯誤/正確性質。
     原因：這113處裡，本次對話近兩輪批次（#299-301的27題、#303-305的42題、#306-307的56題）自己新寫的干擾選項占了絕大多數——反覆使用「反而」表達反轉對照、用「其實」表達糾正常見誤解，是筆者（本次對話AI）本身沒意識到的寫作慣性，如實記錄此點，避免未來又在新一批干擾選項裡不自覺重蹈覆轍；也提醒未來若再新增或修改選項，應以「拿掉所有語氣詞後仍需要實際知識才能判斷對錯」作為額外的自我檢查標準，而不只檢查邏輯是否自相矛盾。
309. **本次修正後執行的收尾驗收**：node腳本讀入完整300題陣列驗證——VM沙盒解析語法通過；300題結構檢查（4選項、無重複）全數通過；300題id全數唯一；全陣列correctIndex連續3題以上同正解位置殘留數為0（分佈0/1/2/3三次驗證皆維持75/76/72/77的均勻分佈，因本輪只置換選項文字、未變動任何correctIndex，故無需重跑重排腳本）；全題庫「其實/反而」殘留數為0。
     原因：延續本專案「收工前跑完整驗收腳本、把數字寫進決策記錄」的既定做法（見#284-291），讓使用者不必自行重新掃描即可確認本輪修正確實涵蓋完整、沒有遺漏或誤改其他題目。
