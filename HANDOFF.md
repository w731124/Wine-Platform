...
- **`data/wine-data.js`**：新增 `WINE_DB.wineStyles[]`（與 `classifications[]` 平行的新頂層陣列），目前僅1筆「氣泡酒 Sparkling Wine」，欄位為 `oneLiner`／`history`／`grapes`／`terroir`／`production`（原本還有 `keyTerms`，使用者回饋不需要後已移除，見下方）。
...
**使用者共審核兩輪**：
- 第一輪回饋兩點：(1) `production` 字數超標與 Cava 陳年月數留白皆「沒關係」，內容文字本身沒問題；(2) 四個區塊視覺上混在一起、標題不夠顯眼。針對第2點修正（DECISIONS.md #76）：把五個區塊（含當時還有的 Key Terms）各自包進既有的 `.ic` 卡片樣式，標題加上圖示並改為 `--burg` 酒紅色。
- 第二輪回饋兩點：(1) `.ic` 卡片顏色跟底色幾乎一樣、辨識度仍不夠——原因是 `.ic` 預設背景 `var(--bg-el)` 剛好跟父層 `.acc-body` 背景相同，等於零對比，修正為在 `winestyles.js` 用行內樣式把卡片背景改成更淺的 `var(--bg-card)`（白色），未更動 `.ic` 全站共用 class 本身；(2) 「關鍵字」資訊不需要——整段移除 Key Terms 區塊，並同步移除 `data/wine-data.js` 裡的 `keyTerms` 欄位避免死資料（DECISIONS.md #77）。

以上已 commit，**尚未 push**。**本次仍只做氣泡酒一款，未套用到紅/白/粉紅/橘酒/強化酒，等使用者確認最終畫面後才決定下一步。**
...
- **本次 session 異動範圍**：`index.html`（啟用釀造工藝導覽項＋新增panel）、`data/wine-data.js`（新增 `wineStyles[]`，僅1筆，後又移除 `keyTerms` 欄位）、`js/winestyles.js`（新檔＋兩輪依使用者回饋修正卡片視覺）、`js/core.js`（DOMContentLoaded加一行）、`DECISIONS.md`、`HANDOFF.md`。
- **資料完整性**：`data/wine-data.js` 現況大括號 1015/1015、中括號 546/546 配對平衡（`keyTerms` 移除後中括號數變化，接手前建議重新跑一次核對而非直接沿用這個數字）；`appellations` 91筆、`grapes` 23筆、`classifications` 7筆、`wineStyles` 1筆（無 `keyTerms` 欄位）。
