...
function openVMI(rid,year,score,st){
  ...
  if(d){
    ...
    <div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl" style="color:var(--gold-dk);font-size:11px;">📊 結構量化 Structure</p>${bH}</div>
    <div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl" style="color:var(--gold-dk);font-size:11px;">🍷 侍酒師建議 Sommelier's Tips</p><div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:11.5px;"><div><span style="color:var(--txt4);">醒酒：</span>${d.sommelier.decant}</div><div><span style="color:var(--txt4);">溫度：</span>${d.sommelier.temp}</div><div><span style="color:var(--txt4);">杯型：</span>${d.sommelier.glass}</div><div><span style="color:var(--txt4);">巔峰：</span>${d.sommelier.peak}</div></div></div>
    <div class="ic" style="background:var(--bg-card);"><p class="ins-lbl" style="color:var(--gold-dk);font-size:11px;">🌸 風味輪廓 Aroma Profile</p><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;">
