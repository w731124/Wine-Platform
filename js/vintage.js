...
function openVMI(rid,year,score,st){
  ...
  if(d){
    ...
    const bH=dims.map(dm=>{const v=d.structure[dm.k]??0;return`<div style="margin-bottom:5px;"><div style="display:flex;justify-content:space-between;font-size:var(--fs-base);margin-bottom:2px;"><span style="color:var(--txt3);">${dm.l}</span><span style="color:var(--burg);font-weight:600;">${v}/10</span></div><div class="stat-bg"><div class="stat-fill" style="width:${v*10}%;background:linear-gradient(90deg,var(--burg),var(--gold));"></div></div></div>`}).join('');
    h+=`<div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl">📝 年份總結 Summary</p><p style="font-size:var(--fs-base);line-height:1.65;color:var(--txt2);">${d.summary}</p></div>
    <div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl">🌦️ 氣候成因 Climate</p><p style="font-size:var(--fs-base);line-height:1.65;color:var(--txt2);">${d.climate}</p></div>
    <div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl">📊 結構量化 Structure</p>${bH}</div>
    <div class="ic mb-3" style="background:var(--bg-card);"><p class="ins-lbl">🍷 侍酒師建議 Sommelier's Tips</p><div class="vmi-grid2" style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:var(--fs-base);color:var(--txt2);"><div><span style="color:var(--txt4);">醒酒：</span>${d.sommelier.decant}</div><div><span style="color:var(--txt4);">溫度：</span>${d.sommelier.temp}</div><div><span style="color:var(--txt4);">杯型：</span>${d.sommelier.glass}</div><div><span style="color:var(--txt4);">巔峰：</span>${d.sommelier.peak}</div></div></div>
    <div class="ic" style="background:var(--bg-card);"><p class="ins-lbl">🌸 風味輪廓 Aroma Profile</p><div class="vmi-grid3" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;">
      <div style="background:rgba(59,110,168,.06);border:1px solid rgba(59,110,168,.15);border-radius:8px;padding:7px;"><p style="font-size:var(--fs-lg);font-weight:700;color:#1A3A6A;margin-bottom:3px;">初級</p><p style="font-size:var(--fs-base);color:var(--txt2);">${d.aromaP}</p></div>
      <div style="background:rgba(92,6,28,.06);border:1px solid rgba(92,6,28,.15);border-radius:8px;padding:7px;"><p style="font-size:var(--fs-lg);font-weight:700;color:var(--burg);margin-bottom:3px;">次級</p><p style="font-size:var(--fs-base);color:var(--txt2);">${d.aromaS}</p></div>
      <div style="background:rgba(42,122,88,.06);border:1px solid rgba(42,122,88,.15);border-radius:8px;padding:7px;"><p style="font-size:var(--fs-lg);font-weight:700;color:var(--em);margin-bottom:3px;">三級</p><p style="font-size:var(--fs-base);color:var(--txt2);">${d.aromaT}</p></div>
    </div></div>`;
  } else {
    h+=`<div class="ic" style="background:var(--bg-card);text-align:center;padding:20px;"><p style="font-size:var(--fs-base);color:var(--txt2);">試試 2005、2010、2015 等年份的深度資料。</p></div>`;
  }
